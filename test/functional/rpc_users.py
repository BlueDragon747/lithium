...
class HTTPBasicsTest(BitcoinTestFramework):
    ...
    def setup_chain(self):
        super().setup_chain()
        # Append rpcauth to lithium.conf before initialization
        ...
        with open(get_config_path(get_datadir_path(self.options.tmpdir, 0)), 'a', encoding='utf8') as f:
            f.write(rpcauth + "\n")
            f.write(rpcauth2 + "\n")
            f.write(rpcauth3 + "\n")
        with open(get_config_path(get_datadir_path(self.options.tmpdir, 1)), 'a', encoding='utf8') as f:
            f.write("rpcuser={}\n".format(self.rpcuser))
