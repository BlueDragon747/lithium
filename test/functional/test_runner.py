...
BASE_SCRIPTS = [
    ...
    'feature_auxpow_mining.py',
    'feature_auxpow_segwit.py',
    'feature_auxpow_pruned_header.py',
    ...
    'interface_lithium_cli.py --legacy-wallet',
    'interface_lithium_cli.py --descriptors',
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if a node daemon is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        running = any(
            subprocess.run(["pgrep", "-x", proc], stdout=subprocess.DEVNULL).returncode == 0
            for proc in ("lithiumd",)
        )
        if running:
            print("%sWARNING!%s There is already a node daemon process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
class RPCCoverage():
    ...
    commands per `lithium-cli help` (`rpc_interface.txt`).
