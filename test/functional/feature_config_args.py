...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that a legacy default bitcoin.conf is ignored unless it is explicitly selected.
        legacy_conf_file_path = os.path.join(self.nodes[0].datadir, 'bitcoin.conf')
        with open(legacy_conf_file_path, 'w', encoding='utf-8') as conf:
            conf.write('conf=some.conf\n')
        self.start_node(0)
        self.stop_node(0)
        self.nodes[0].assert_start_raises_init_error(
            extra_args=[f'-conf={legacy_conf_file_path}'],
            expected_msg=conf_in_config_file_err,
        )

        # Check that startup fails if conf= is set in lithium.conf or in an included conf file
        bad_conf_file_path = os.path.join(self.options.tmpdir, 'node0', 'lithium_bad.conf')
        ...
        with open(util.get_config_path(self.nodes[0].datadir), 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = os.path.join(self.options.tmpdir, 'node0', 'lithium_main.conf')
    ...
    def run_test(self):
        ...
        conf_file = util.get_config_path(default_data_dir)
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_lithium.conf")
