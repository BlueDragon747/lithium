...
from test_framework.blocktools import TAPROOT_WALLET_FUND_BLOCKS
...
for entry in KEYS:
    entry["xprv"] = translate_testnet_extkey(entry["xprv"])
    entry["xpub"] = translate_testnet_extkey(entry["xpub"])
...
class WalletTaprootTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Lithium regtest uses the historical 100-block maturity below the
        # mainnet restore height. Mine one extra block beyond
        # the nominal boundary so the funding wallet has spendable outputs on
        # both descriptor and legacy harness paths before the Taproot spend loop.
        self.generatetoaddress(self.nodes[0], TAPROOT_WALLET_FUND_BLOCKS, gen_addr, sync_fun=self.no_op)
