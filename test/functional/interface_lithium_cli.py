...
# Lithium regtest uses the 0.8 subsidy ladder from genesis. Mine enough mature
# coinbase outputs for the wallet send checks below instead of assuming 50 LIT
# rewards.
BLOCKS = COINBASE_MATURITY + 120
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `lithium-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and lithium-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and lithium-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Balance']), self.nodes[0].getwalletinfo()["balance"])
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (LIT/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (LIT/kvB)']), network_info['relayfee'])
            assert_equal(self.nodes[0].cli.getwalletinfo(), wallet_info)

            # Setup to test -getinfo, -generate, and -rpcwallet= with multiple wallets.
            wallets = [self.default_wallet_name, 'Encrypted', 'secret']
            amounts = [Decimal(0), Decimal(9), Decimal(31)]
            ...
            # Mine a block to confirm the sends.
            self.generate(self.nodes[0], 1)
            amounts[0] = w1.getbalance()
            amounts[1] = w2.getbalance()
            amounts[2] = w3.getbalance()
            ...
            # Test lithium-cli -generate with -rpcwallet in multiwallet mode.
