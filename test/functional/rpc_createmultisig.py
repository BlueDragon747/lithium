...
class RpcCreateMultiSigTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for t in vectors:
            ...
            assert_equal(self.nodes[0].deriveaddresses(desc)[0], translate_testnet_address(t['address']))
            sorted_key_str = ','.join(t['sorted_keys'])
            sorted_key_desc = descsum_create('sh(multi(2,{}))'.format(sorted_key_str))
            assert_equal(self.nodes[0].deriveaddresses(sorted_key_desc)[0], translate_testnet_address(t['address']))
    ...
    def do_multisig(self):
        ...
        if self.output_type == 'bech32':
            assert madd.startswith("rlit1")  # actually a bech32 address
