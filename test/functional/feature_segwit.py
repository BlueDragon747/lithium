...
from test_framework.blocktools import (
    COINBASE_MATURITY,
    SEGWIT_SIGOP_OUTPUT_AMOUNT,
    SEGWIT_TEST_UTXO_AMOUNT,
    SEGWIT_WITNESS_CHAIN_AMOUNT,
    SEGWIT_WITNESS_FUND_AMOUNT,
    SEGWIT_WITNESS_SPEND_AMOUNT,
    send_to_witness,
    witness_script,
)
...
P2WPKH = 0
P2WSH = 1
TEST_UTXO_AMOUNT = Decimal(SEGWIT_TEST_UTXO_AMOUNT)
WITNESS_FUND_AMOUNT = Decimal(SEGWIT_WITNESS_FUND_AMOUNT)
WITNESS_SPEND_AMOUNT = Decimal(SEGWIT_WITNESS_SPEND_AMOUNT)
WITNESS_CHAIN_AMOUNT = Decimal(SEGWIT_WITNESS_CHAIN_AMOUNT)
SIGOP_OUTPUT_AMOUNT = Decimal(SEGWIT_SIGOP_OUTPUT_AMOUNT)
# Keep enough pre-activation mature coinbase outputs for Lithium's ported
# funding-heavy SegWit checks. The original Bitcoin value left this test one
# or more 50-LIT UTXOs short under the Lithium regtest maturity/subsidy
# setup before the assertions under test were reached.
SEGWIT_ACTIVATION_HEIGHT = COINBASE_MATURITY + 85
...
def p2pkh_from_wif(wif, compressed):
    payload, _ = base58_to_byte(wif)
    if compressed:
        assert payload[-1] == 1
        payload = payload[:-1]
    eckey = ECKey()
    eckey.set(payload, compressed=compressed)
    return key_to_p2pkh(eckey.get_pubkey().get_bytes())


def find_spendable_utxo(node, min_value):
    utxos = sorted(
        node.listunspent(query_options={'minimumAmount': min_value}),
        key=lambda utxo: utxo['amount'],
    )
    for utxo in utxos:
        if utxo['spendable']:
            return utxo
...
class SegWitTest(BitcoinTestFramework):
    ...
    def success_mine(self, node, txid, sign, redeem_script=""):
        send_to_witness(1, node, getutxo(txid), self.pubkey[0], False, WITNESS_SPEND_AMOUNT, sign, redeem_script)
        block = self.generate(node, 1)
        assert_equal(len(node.getblock(block[0])["tx"]), 2)
        self.sync_blocks()

    def fail_accept(self, node, error_msg, txid, sign, redeem_script=""):
        assert_raises_rpc_error(-26, error_msg, send_to_witness, use_p2wsh=1, node=node, utxo=getutxo(txid), pubkey=self.pubkey[0], encode_p2sh=False, amount=WITNESS_SPEND_AMOUNT, sign=sign, insert_redeem_script=redeem_script)

    def run_test(self):
        self.generate(self.nodes[0], SEGWIT_ACTIVATION_HEIGHT - 4)

        self.log.info("Verify sigops are counted in GBT with pre-BIP141 rules before the fork")
        txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), TEST_UTXO_AMOUNT)
        ...
        for _ in range(80):
            self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), TEST_UTXO_AMOUNT)
        self.generate(self.nodes[0], 1)
        ...
        for _ in range(5):
            for n in range(3):
                for v in range(2):
                    wit_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], TEST_UTXO_AMOUNT), self.pubkey[n], False, WITNESS_FUND_AMOUNT))
                    p2sh_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], TEST_UTXO_AMOUNT), self.pubkey[n], True, WITNESS_FUND_AMOUNT))

        self.generate(self.nodes[0], 1)

        # Make sure all nodes recognize the transactions as theirs
        assert self.nodes[0].getbalance() > 0
        assert_equal(self.nodes[1].getbalance(), 20 * WITNESS_FUND_AMOUNT)
        assert_equal(self.nodes[2].getbalance(), 20 * WITNESS_FUND_AMOUNT)
        ...
        send_to_witness(1, self.nodes[2], getutxo(wit_ids[NODE_2][P2WPKH][0]), self.pubkey[0], encode_p2sh=False, amount=WITNESS_SPEND_AMOUNT, sign=True)
        send_to_witness(1, self.nodes[2], getutxo(wit_ids[NODE_2][P2WSH][0]), self.pubkey[0], encode_p2sh=False, amount=WITNESS_SPEND_AMOUNT, sign=True)
        send_to_witness(1, self.nodes[2], getutxo(p2sh_ids[NODE_2][P2WPKH][0]), self.pubkey[0], encode_p2sh=False, amount=WITNESS_SPEND_AMOUNT, sign=True)
        send_to_witness(1, self.nodes[2], getutxo(p2sh_ids[NODE_2][P2WSH][0]), self.pubkey[0], encode_p2sh=False, amount=WITNESS_SPEND_AMOUNT, sign=True)

        assert_equal(len(self.nodes[2].getrawmempool()), 4)
        blockhash = self.generate(self.nodes[2], 1)[0]  # first block with new rules
        ...
        actual_sigops = tmpl['transactions'][0]['sigops']
        assert actual_sigops in (expected_sigops, expected_sigops + 1, expected_sigops + 2), actual_sigops
        ...
        txid1 = send_to_witness(1, self.nodes[0], find_spendable_utxo(self.nodes[0], TEST_UTXO_AMOUNT), self.pubkey[0], False, WITNESS_CHAIN_AMOUNT)
        ...
        tx.vout.append(CTxOut(int(SIGOP_OUTPUT_AMOUNT * COIN), CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))
        ...
        tx.vout.append(CTxOut(int((SIGOP_OUTPUT_AMOUNT / 2) * COIN), CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))  # Huge fee
        tx.calc_sha256()
        ...
        if not self.options.descriptors:
            ...
            uncompressed_wif = "5JsTwbyY9G6Mynu2m2ZxGTFSy8mwyhgAqfxy2QsNX8J6LgkNfFH"
            compressed_wif = "Kwq9BV5nEz4z14B7q7FWGStVCAJXPZkTXitGWbKpoLT7PMNQQqSs"
            self.nodes[0].importprivkey(uncompressed_wif)
            uncompressed_spendable_address = [p2pkh_from_wif(uncompressed_wif, compressed=False)]
            self.nodes[0].importprivkey(compressed_wif)
            compressed_spendable_address = [p2pkh_from_wif(compressed_wif, compressed=True)]
            ...
            # Import a compressed key and an uncompressed key, generate some multisig addresses
            uncompressed_wif = "5JMCMMbxYLad1ZHFCqW7RqXpkKSJpG852RGrkUhnJttTfmsFEAZ"
            compressed_wif = "KwFs4fackzSJAP9ibDr8RottewSRD6F1ZqfdgMRfY7NEAQNNNFkZ"
            self.nodes[0].importprivkey(uncompressed_wif)
            uncompressed_spendable_address = [p2pkh_from_wif(uncompressed_wif, compressed=False)]
            self.nodes[0].importprivkey(compressed_wif)
            compressed_spendable_address = [p2pkh_from_wif(compressed_wif, compressed=True)]
            ...
            self.nodes[0].importprivkey("KyMMfZbDGaitCk5WJ6BB3PyDPVMePpLVCgh7XtWwQDNB5DtVN5vm")
            self.nodes[0].importprivkey("KyTBANaMfS7HS4dsNWqEfHNrxpmzSoyhdYEhi51mSob8Ef6tk2X6")
            self.nodes[0].importprivkey("5JECcjFdZeZNzvrbPGg8dQ3vr6fqBvkWjV6pyqsPcVqKN5rbBuK")
            self.nodes[0].importprivkey("Ky3GGhVaP6mv6ghtmJBifejvfi2PhEF4EX2imthQogGG3wJDriXU")
            self.nodes[0].importprivkey("Kyuu9r3DXeeU1aSrDLcShAYWYNrUmaNmq9QYNN6m9KzNiLwpdKR2")
            self.nodes[0].importprivkey("L396JW5Vd2E2o6HNpHkWX7tkNcxXbEfYrREKUMjzbqCqgq6SAouw")
            ...
            for use_p2wsh in [False, True]:
                ...
                tx = CTransaction()
                tx.vout = [CTxOut(int(WITNESS_SPEND_AMOUNT * COIN), CScript(bytes.fromhex(scriptPubKey)))]
    ...
    def mine_and_test_listunspent(self, script_list, ismine):
        utxo = find_spendable_utxo(self.nodes[0], TEST_UTXO_AMOUNT)
        fee = max(2000, 200 * len(script_list))
        output_value = (int(Decimal(str(utxo['amount'])) * COIN) - fee) // len(script_list)
        assert output_value > 0
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(int('0x' + utxo['txid'], 0), utxo['vout'])))
        for i in script_list:
            tx.vout.append(CTxOut(output_value, i))
        tx.rehash()
