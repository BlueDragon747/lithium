...
class PSBTTest(BitcoinTestFramework):
    ...
    def _reference_txid_map(self, vectors):
        txid_map = {}
        groups = (
            vectors["valid"],
            [creator["result"] for creator in vectors["creator"]],
            [signer["psbt"] for signer in vectors["signer"]],
            [signer["result"] for signer in vectors["signer"]],
            [psbt for combiner in vectors["combiner"] for psbt in combiner["combine"]],
            [combiner["result"] for combiner in vectors["combiner"]],
            [finalizer["finalize"] for finalizer in vectors["finalizer"]],
            [finalizer["result"] for finalizer in vectors["finalizer"]],
            [extractor["extract"] for extractor in vectors["extractor"]],
        )
        for group in groups:
            for candidate in group:
                try:
                    psbt = PSBT.from_base64(candidate)
                except Exception:
                    continue
                for txin, input_map in zip(psbt.tx.vin, psbt.i):
                    if PSBT_IN_NON_WITNESS_UTXO not in input_map.map:
                        continue
                    prev_tx = CTransaction()
                    prev_tx.deserialize(BytesIO(input_map.map[PSBT_IN_NON_WITNESS_UTXO]))
                    prev_tx.rehash()
                    txid_map[f"{txin.prevout.hash:064x}"] = prev_tx.hash
        return txid_map

    def _lithiumize_reference_psbt(self, psbt_b64, txid_map):
        psbt = PSBT.from_base64(psbt_b64)
        changed = False
        for txin, input_map in zip(psbt.tx.vin, psbt.i):
            old_txid = f"{txin.prevout.hash:064x}"
            if PSBT_IN_NON_WITNESS_UTXO in input_map.map:
                prev_tx = CTransaction()
                prev_tx.deserialize(BytesIO(input_map.map[PSBT_IN_NON_WITNESS_UTXO]))
                prev_tx.rehash()
                new_txid = prev_tx.hash
            else:
                new_txid = txid_map.get(old_txid)
            if new_txid is None or new_txid == old_txid:
                continue
            txin.prevout.hash = int(new_txid, 16)
            changed = True
        if changed:
            psbt.g.map[PSBT_GLOBAL_UNSIGNED_TX] = psbt.tx.serialize_without_witness()
        return psbt.to_base64()

    def _maybe_lithiumize_reference_psbt(self, psbt_b64, txid_map):
        try:
            return self._lithiumize_reference_psbt(psbt_b64, txid_map)
        except Exception:
            return psbt_b64

    def _lithiumize_reference_rawtx(self, rawtx_hex, txid_map):
        tx = CTransaction()
        tx.deserialize(BytesIO(bytes.fromhex(rawtx_hex)))
        changed = False
        for txin in tx.vin:
            new_txid = txid_map.get(f"{txin.prevout.hash:064x}")
            if new_txid is None:
                continue
            txin.prevout.hash = int(new_txid, 16)
            changed = True
        return tx.serialize().hex() if changed else rawtx_hex

    def _lithiumize_reference_address(self, address):
    ...
    def _decode_legacy_base58check(self, value):
        n = 0
        for char in value:
            n *= 58
            n += b58chars.index(char)
        ...
        if hash256(decoded[:-4])[:4] != decoded[-4:]:
            raise ValueError('Invalid legacy Base58Check checksum')
        return decoded[1:-4], decoded[0]

    def _lithiumize_reference_wif(self, wif):
        try:
            payload, _version = base58_to_byte(wif)
        except ValueError:
            payload, _version = self._decode_legacy_base58check(wif)
        return byte_to_base58(payload, 128)

    def _psbt_signature_shape(self, psbt_b64):
        psbt = PSBT.from_base64(psbt_b64)
        shape = []
        for input_map in psbt.i:
            partial_sig_pubkeys = sorted(
                key[1:].hex()
                for key in input_map.map
                if isinstance(key, bytes) and len(key) > 1 and key[0] == PSBT_IN_PARTIAL_SIG
            )
            shape.append({
                "partial_sig_pubkeys": partial_sig_pubkeys,
                "has_final_scriptsig": PSBT_IN_FINAL_SCRIPTSIG in input_map.map,
                "has_final_scriptwitness": PSBT_IN_FINAL_SCRIPTWITNESS in input_map.map,
            })
        return shape

    def _load_reference_vectors(self):
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data/rpc_psbt.json'), encoding='utf-8') as f:
            vectors = json.load(f)

        txid_map = self._reference_txid_map(vectors)
        self.reference_psbt_txid_map = txid_map

        vectors["valid"] = [
            self._maybe_lithiumize_reference_psbt(psbt, txid_map)
            for psbt in vectors["valid"]
        ]
        ...
        vectors["creator"] = [
            {
                "inputs": [
                    {
                        "txid": txid_map.get(txin["txid"], txin["txid"]),
                        "vout": txin["vout"],
                    }
                ...
                "outputs": [
                    {
                        self._lithiumize_reference_address(address): amount
                        for address, amount in output.items()
                    }
                    for output in creator["outputs"]
                ],
                "result": self._maybe_lithiumize_reference_psbt(creator["result"], txid_map),
            }
            for creator in vectors["creator"]
        ]

        vectors["signer"] = [
            {
                "privkeys": [self._lithiumize_reference_wif(key) for key in signer["privkeys"]],
                "psbt": self._maybe_lithiumize_reference_psbt(signer["psbt"], txid_map),
                "result": self._maybe_lithiumize_reference_psbt(signer["result"], txid_map),
            }
            for signer in vectors["signer"]
        ]

        vectors["combiner"] = [
            {
                "combine": [self._maybe_lithiumize_reference_psbt(psbt, txid_map) for psbt in combiner["combine"]],
                "result": self._maybe_lithiumize_reference_psbt(combiner["result"], txid_map),
            }
            for combiner in vectors["combiner"]
        ]

        vectors["finalizer"] = [
            {
                "finalize": self._maybe_lithiumize_reference_psbt(finalizer["finalize"], txid_map),
                "result": self._maybe_lithiumize_reference_psbt(finalizer["result"], txid_map),
            }
            for finalizer in vectors["finalizer"]
        ]

        vectors["extractor"] = [
            {
                "extract": self._maybe_lithiumize_reference_psbt(extractor["extract"], txid_map),
                "result": self._lithiumize_reference_rawtx(extractor["result"], txid_map),
            }
            for extractor in vectors["extractor"]
        ]
    ...
    def run_test(self):
        # Create and fund a raw tx for sending 10 LIT
        ...
        assert_raises_rpc_error(-4, "The preselected coins total amount does not cover the transaction target. "
                                ...
                                self.nodes[0].walletcreatefundedpsbt, [{"txid": utxo1['txid'], "vout": utxo1['vout']}], {self.nodes[2].getnewaddress():1})

        psbtx1 = self.nodes[0].walletcreatefundedpsbt([{"txid": utxo1['txid'], "vout": utxo1['vout']}], {self.nodes[2].getnewaddress():1}, 0, {"add_inputs": True})['psbt']
        ...
        utxo1 = self.nodes[0].listunspent()[0]
        psbtx1 = self.nodes[0].walletcreatefundedpsbt([{"txid": utxo1['txid'], "vout": utxo1['vout']}], {self.nodes[2].getnewaddress():0.1}, 0,{"lockUnspents": True})["psbt"]
        assert_equal(len(self.nodes[0].listlockunspent()), 1)

        # Locks are ignored for manually selected inputs
        self.nodes[0].walletcreatefundedpsbt([{"txid": utxo1['txid'], "vout": utxo1['vout']}], {self.nodes[2].getnewaddress():0.1}, 0)
        ...
        outputs = [{self.nodes[1].getnewaddress(): 0.59}]
        ...
        self.log.info("Test walletcreatefundedpsbt fee rate of 10000 sat/vB and 0.1 LIT/kvB produces a total fee at or slightly below -maxtxfee (~0.05290000)")
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (LIT/kvB)",
        ...
        for bool_add, outputs_array in {True: outputs, False: [{self.nodes[1].getnewaddress(): 0.1}]}.items():
        ...
        psbtx = wmulti.walletcreatefundedpsbt(inputs=[{"txid":txid,"vout":p2wsh_pos},{"txid":txid,"vout":p2sh_pos},{"txid":txid,"vout":p2sh_p2wsh_pos}], outputs={self.nodes[1].getnewaddress():0.59}, options={'changeAddress': self.nodes[1].getrawchangeaddress()})['psbt']
        ...
        rawtx = self.nodes[1].createrawtransaction([{"txid":txid,"vout":p2wpkh_pos}], {self.nodes[1].getnewaddress():0.19})
        ...
        d = self._load_reference_vectors()
        invalids = d['invalid']
        ...
        for i, signer in enumerate(signers):
            ...
            assert_equal(self.nodes[0].decodepsbt(signed_tx)["tx"], self.nodes[0].decodepsbt(signer['result'])["tx"])
            assert_equal(self._psbt_signature_shape(signed_tx), self._psbt_signature_shape(signer['result']))

        # Combiner test
        for combiner in combiners:
            combined = self.nodes[2].combinepsbt(combiner['combine'])
            assert_equal(self.nodes[0].decodepsbt(combined)["tx"], self.nodes[0].decodepsbt(combiner['result'])["tx"])
            assert_equal(self._psbt_signature_shape(combined), self._psbt_signature_shape(combiner['result']))
        ...
        for finalizer in finalizers:
            ...
            assert_equal(self.nodes[0].decodepsbt(finalized)["tx"], self.nodes[0].decodepsbt(finalizer['result'])["tx"])

        # Extractor test
        for extractor in extractors:
            self.nodes[0].decodepsbt(extractor['extract'])
        ...
        analysis = self.nodes[0].analyzepsbt(self._maybe_lithiumize_reference_psbt('', self.reference_psbt_txid_map))
        assert_equal(analysis['next'], 'creator')
        assert_equal(analysis['error'], 'PSBT is not valid. Input 0 spends unspendable output')

        self.log.info("PSBT with invalid values should have error message and Creator as next")
        analysis = self.nodes[0].analyzepsbt(self._maybe_lithiumize_reference_psbt('cHNidP8BAHECAAAAAfA00BFgAm6tp86RowwH6BMImQNL5zXUcTT97XoLGz0BAAAAAAD/////AgD5ApUAAAAAFgAUKNw0x8HRctAgmvoevm4u1SbN7XL87QKVAAAAABYAFPck4gF7iL4NL4wtfRAKgQbghiTUAAAAAAABAR8AgIFq49AHABYAFJUDtxf2PHo641HEOBOAIvFMNTr2AAAA', self.reference_psbt_txid_map))
        assert_equal(analysis['next'], 'creator')
        assert_equal(analysis['error'], 'PSBT is not valid. Input 0 has invalid value')

        self.log.info("PSBT with signed, but not finalized, inputs should have Finalizer as next")
        analysis = self.nodes[0].analyzepsbt(self._maybe_lithiumize_reference_psbt('', self.reference_psbt_txid_map))
        ...
        invalid_prevout_psbt = self._maybe_lithiumize_reference_psbt('', self.reference_psbt_txid_map)
        analysis = self.nodes[0].analyzepsbt(invalid_prevout_psbt)
        assert_equal(analysis['next'], 'creator')
        assert_equal(analysis['error'], 'PSBT is not valid. Input 0 specifies invalid prevout')

        assert_raises_rpc_error(-25, 'Inputs missing or spent', self.nodes[0].walletprocesspsbt, invalid_prevout_psbt)
