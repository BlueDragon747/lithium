...
class P2PIBDStallingTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        blocks = []
        height = 1
        # Lithium regtest inherits the genesis target until a block is more than
        # 2 * spacing ahead of the tip. The handcrafted blocks below use the
        # regtest powLimit nBits from blocktools.create_block(), so start beyond
        # that window to keep the whole header chain on the easy-target path.
        block_time = node.getblock(node.getbestblockhash())['time'] + (2 * 3 * 60) + 1
        ...
        expected_block_bytes = sum(len(msg_block(b).serialize()) for b in blocks[1:NUM_BLOCKS-1])
        peers = []
        ...
        # Wait until 1023 blocks are received. Lithium's block payload sizes differ from
        # Bitcoin's historical fixture, so derive the byte target from the generated blocks.
        self.wait_until(lambda: self.total_bytes_recv_for_blocks() >= expected_block_bytes)
