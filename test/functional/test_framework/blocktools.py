...
# Genesis block time (Lithium regtest — same nTime as Lithium mainnet)
TIME_GENESIS_BLOCK = 1411788333
...
TAPROOT_WALLET_FUND_BLOCKS = COINBASE_MATURITY + 2
TAPROOT_STRESS_FUND_BLOCKS = COINBASE_MATURITY + 120
CSV_SELF_TRANSFER_FEE_RATE = "0.003"
SEGWIT_TEST_UTXO_AMOUNT = "0.01"
SEGWIT_WITNESS_FUND_AMOUNT = "0.009"
SEGWIT_WITNESS_SPEND_AMOUNT = "0.008"
SEGWIT_WITNESS_CHAIN_AMOUNT = "0.006"
SEGWIT_SIGOP_OUTPUT_AMOUNT = "0.005"


def get_coinbase_value(height):
    if height == 0:
        return 5 * COIN
    # Lithium regtest leaves the 0.8 subsidy ladder active from genesis.
    if height < 1999:
        return 48 * (COIN // 100)
    if height < 175000:
        return 48 * COIN
...
def coinbase_maturity(spend_height):
...
def create_coinbase(height, pubkey=None, *, script_pubkey=None, extra_output_script=None, fees=0, nValue=None):
    ...
    coinbaseoutput.nValue = get_coinbase_value(height) if nValue is None else int(nValue * COIN)
