#!/usr/bin/env python3
# Copyright (c) 2017-2022 The Lithium Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Class for lithiumd node under test"""
...
class TestNode():
    """A class for representing a lithiumd node under test.
    ...
    def __init__(self, i, datadir, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        ...
        self.bitcoinconf = get_config_path(self.datadir)
        ...
        # Configuration for logging is set as command-line args rather than in the lithium.conf file.
        # This means that starting a lithiumd using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
    ...
    @classmethod
    def deterministic_address_key_pair(cls, index, chain="regtest"):
        """Return a deterministic chain-valid address/WIF pair for a node index."""
        secret = (index + 1).to_bytes(32, 'big')
        eckey = ECKey()
        eckey.set(secret, compressed=True)
        ...
        if chain == "signet":
            ...
            address_version = 19
            secret_version = 128
        address = cls._encode_base58check(hash160(pubkey), address_version)
        wif = cls._encode_base58check(secret + b'\x01', secret_version)
        return cls.AddressKeyPair(address, wif)

    def get_deterministic_priv_key(self):
        """Return a deterministic chain-valid address/WIF pair derived from the node index."""
        return self.deterministic_address_key_pair(self.index, self.chain)

    @staticmethod
    def _encode_base58check(payload: bytes, version: int) -> str:
        data = bytes([version]) + payload
        data += blake256_hash(data)[:4]
    ...
    def __del__(self):
        # Ensure that we don't leave any lithiumd processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time lithiumd is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by lithiumd, and
        ...
        self.running = True
        self.log.debug("lithiumd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the lithiumd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'lithiumd exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; lithiumd is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to lithiumd after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; lithiumd is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into lithiumd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to lithiumd
        expected_msg: regex that stderr should match when lithiumd fails

        Will throw if lithiumd starts without an error.
        Will throw if an expected_msg is provided and it does not match lithiumd's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'lithiumd exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'lithiumd should have exited within {self.rpc_timeout}s '
    ...
    def add_p2p_connection(self, p2p_conn, *, wait_for_verack=True, **kwargs):
        ...
        if wait_for_verack:
            ...
            # Consistency check that the Lithium Core has received our user agent string. This checks the
            # node's newest peer. It could be racy if another Lithium Core node has connected since we opened
...
class TestNodeCLI():
    """Interface to lithium-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with lithium-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run lithium-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running lithium-cli {}".format(p_args[2:]))
