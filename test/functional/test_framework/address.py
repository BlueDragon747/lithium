...
ADDRESS_BCRT1_UNSPENDABLE = 'rlit1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqmsqlzs'
ADDRESS_BCRT1_UNSPENDABLE_DESCRIPTOR = 'addr(rlit1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqmsqlzs)#amecmc09'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE
ADDRESS_BCRT1_P2WSH_OP_TRUE = 'rlit1qft5p2uhsdcdc3l2ua4ap5qqfg4pjaqlp250x7us7a8qqhrxrxfsq60p7pk'
...
PUBKEY_ADDRESS_VERSION = 19
SCRIPT_ADDRESS_VERSION = 7
TESTNET_PUBKEY_ADDRESS_VERSION = 142
TESTNET_SCRIPT_ADDRESS_VERSION = 170
MAIN_HRP = 'lit'
TESTNET_HRP = 'tlit'
REGTEST_HRP = 'rlit'

# Lithium regtest/mainnet use xpub/xprv version bytes with Blake Base58Check
# (inherited Lithium-family Base58 identity).
TESTNET_TO_BLAKE_EXTKEY = {
    bytes.fromhex("043587cf"): bytes.fromhex("0488b21e"),
    bytes.fromhex("04358394"): bytes.fromhex("0488ade4"),
}

TESTNET_TO_BLAKE_WIF = {
    239: 128,
}
...
def create_deterministic_address_bcrt1_p2tr_op_true():
    """
    Generates a deterministic Lithium regtest bech32m address (segwit v1 output) that
    ...
    assert_equal(address, 'rlit1p9yfmy5h72durp7zrhlw9lf7jpwjgvwdg0jr0lqmmjtgg83266lqsmglc06')
    return (address, internal_key)


def _base58check_encode(payload, version_bytes, checksum_fn):
    result = ''
    data = version_bytes + payload
    data += checksum_fn(data)[:4]
...
def _base58check_decode(s, checksum_fn, version_length):
    if not s:
        return b'', b''
    n = 0
    for c in s:
    ...
    if checksum_fn(res[:-4])[:4] != res[-4:]:
        raise ValueError('Invalid Base58Check checksum')

    return res[version_length:-4], res[:version_length]


def byte_to_base58_with_checksum(b, version, checksum_fn):
    version_bytes = bytes([version]) if isinstance(version, int) else bytes(version)
    return _base58check_encode(b, version_bytes, checksum_fn)


def base58_to_byte_with_checksum(s, checksum_fn, version_length=1):
    payload, version = _base58check_decode(s, checksum_fn, version_length)
    if version_length == 1:
        return payload, int(version[0])
    return payload, version


def byte_to_base58(b, version):
    return byte_to_base58_with_checksum(b, version, blake256_hash)
...
def base58_to_byte(s):
    ...
    return base58_to_byte_with_checksum(s, blake256_hash)


def byte_to_legacy_base58(b, version):
    return byte_to_base58_with_checksum(b, version, hash256)


def legacy_base58_to_byte(s, version_length=1):
    return base58_to_byte_with_checksum(s, hash256, version_length)
...
def translate_testnet_wif(wif):
    payload, version = legacy_base58_to_byte(wif)
    return byte_to_base58_with_checksum(payload, TESTNET_TO_BLAKE_WIF[version], blake256_hash)


def keyhash_to_p2pkh(hash, main=False):
    assert len(hash) == 20
    version = PUBKEY_ADDRESS_VERSION
...
def program_to_witness(version, program, main=False):
    ...
    return encode_segwit_address(MAIN_HRP if main else REGTEST_HRP, version, program)
...
def bech32_to_bytes(address):
    ...
    if hrp not in ['bc', 'tb', 'bcrt', MAIN_HRP, TESTNET_HRP, REGTEST_HRP]:
        return (None, None)
...
def address_to_scriptpubkey(address):
    ...
    if version in (PUBKEY_ADDRESS_VERSION, TESTNET_PUBKEY_ADDRESS_VERSION, 111):
        return keyhash_to_p2pkh_script(payload)
    elif version in (SCRIPT_ADDRESS_VERSION, TESTNET_SCRIPT_ADDRESS_VERSION, 196):
