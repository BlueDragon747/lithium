...
TMPDIR_PREFIX = "lithium_func_test_"
CACHE_LOCK_FILENAME = ".functional_cache.lock"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave lithiumd processes and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop lithiumd processes after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use lithium-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        def preferred_binary_path(preferred_name, legacy_name):
            preferred = os.path.join(
                config["environment"]["BUILDDIR"],
                "src",
                preferred_name + config["environment"]["EXEEXT"],
            )
            legacy = os.path.join(
                config["environment"]["BUILDDIR"],
                "src",
                legacy_name + config["environment"]["EXEEXT"],
            )
            return preferred if os.path.exists(preferred) or not os.path.exists(legacy) else legacy

        fname_bitcoind = preferred_binary_path("lithiumd", "lithiumd")
        fname_bitcoincli = preferred_binary_path("lithium-cli", "lithium-cli")
        fname_bitcoinutil = preferred_binary_path("lithium-util", "lithium-util")
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: lithiumd processes were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if self.bind_to_localhost_only:
            extra_confs = [[f"bind=127.0.0.1:{p2p_port(i)}"] for i in range(num_nodes)]
        else:
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'lithiumd', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'lithium-cli', self.options.bitcoincli) for v in versions]
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple lithiumd test nodes"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a lithiumd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple lithiumd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as lithiumd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        cache_ready = os.path.join(self.options.cachedir, f".{self.chain}.cache_ready")
        assert self.num_nodes <= MAX_NODES

        os.makedirs(self.options.cachedir, exist_ok=True)
        cache_lock = os.path.join(self.options.cachedir, CACHE_LOCK_FILENAME)
        with open(cache_lock, "a+", encoding="utf8") as lockfile:
            fcntl.flock(lockfile, fcntl.LOCK_EX)

            rebuild_cache = not os.path.isdir(cache_node_dir) or not os.path.isfile(cache_ready)
            if rebuild_cache and os.path.isdir(cache_node_dir):
                self.log.debug("Removing stale cache directory {}".format(cache_node_dir))
            ...
            if rebuild_cache:
                ...
                self.nodes.append(
                    TestNode(
                        ...
                        cache_node_dir,
                        chain=self.chain,
                        extra_conf=[f"bind=127.0.0.1:{p2p_port(CACHE_NODE_ID)}"],
                ...
                # Set mocktime just beyond Lithium regtest's 2*spacing window so
                # the first post-genesis block drops to powLimit difficulty. After
                # that, Lithium regtest keeps the easy target while mocktime
                # advances from the current tip.
                cache_node.setmocktime(cache_node.getblockheader(cache_node.getbestblockhash())['time'] + (2 * 3 * 60) + 1)
                ...
                gen_addresses = [TestNode.deterministic_address_key_pair(i).address for i in range(3)] + [create_deterministic_address_bcrt1_p2tr_op_true()[0]]
                assert_equal(len(gen_addresses), 4)
                for i in range(8):
                    address = gen_addresses[i % len(gen_addresses)]
                    blocks_to_generate = 25 if i != 7 else 24
                    for _ in range(blocks_to_generate):
                        ...
                        tip = cache_node.getblockheader(cache_node.getbestblockhash())
                        cache_node.setmocktime(tip['time'])
                ...
                with open(cache_ready, "w", encoding="utf8") as ready_file:
                    ready_file.write("199\n")
            else:
                self.log.debug("Using cached chain directory {}".format(cache_node_dir))

        for i in range(self.num_nodes):
            self.log.debug("Copy cache directory {} to node {}".format(cache_node_dir, i))
            to_dir = get_datadir_path(self.options.tmpdir, i)
            shutil.copytree(cache_node_dir, to_dir)
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in lithium.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if lithiumd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("lithiumd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if lithiumd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("lithiumd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if lithium-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("lithium-wallet has not been compiled")

    def skip_if_no_bitcoin_util(self):
        """Skip the running test if lithium-util has not been compiled."""
        if not self.is_bitcoin_util_compiled():
            raise SkipTest("lithium-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if lithium-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("lithium-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether lithium-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether lithium-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoin_util_compiled(self):
        """Checks whether lithium-util was compiled."""
