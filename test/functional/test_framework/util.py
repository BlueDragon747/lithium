...
def assert_fee_amount(fee, tx_size, feerate_BTC_kvB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s LIT too low! (Should be %s LIT)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BTC_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s LIT too high! (Should be %s LIT)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting LIT values"""
...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in LIT given a feerate is LIT/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_btc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  LIT
...
CONFIG_FILENAME = "lithium.conf"


def get_config_path(datadir):
    return os.path.join(datadir, CONFIG_FILENAME)
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, CONFIG_FILENAME), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def append_config(datadir, options):
    with open(get_config_path(datadir), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    config_path = get_config_path(datadir)
