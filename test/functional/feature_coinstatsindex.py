...
class CoinStatsIndexTest(BitcoinTestFramework):
    ...
    def subsidy_amount(self, height):
        return Decimal(get_coinbase_value(height)) / COIN

    def block_sanity_check(self, block_info, height):
        block_subsidy = self.subsidy_amount(height)
        assert_equal(
    ...
    def _test_coin_stats_index(self):
        ...
        initial_tx_height = COINBASE_MATURITY + 2
        ...
        for hash_option in index_hash_options:
            ...
            res2 = index_node.gettxoutsetinfo(hash_option, initial_tx_height)
            ...
            assert_raises_rpc_error(-8, "Querying specific block heights requires coinstatsindex", node.gettxoutsetinfo, hash_option, initial_tx_height)

        self.log.info("Test gettxoutsetinfo() with index and verbose flag")

        for hash_option in index_hash_options:
            # Genesis block is unspendable
            genesis_subsidy = self.subsidy_amount(0)
            ...
            self.block_sanity_check(res4['block_info'], 0)

            # Test an older block height that included a normal tx
            res5 = index_node.gettxoutsetinfo(hash_option, initial_tx_height)
            assert_equal(res5['total_unspendable_amount'], genesis_subsidy)
            assert_equal(res5['block_info']['unspendable'], 0)
            assert_equal(res5['block_info']['unspendables'], {
                'genesis_block': 0,
                'bip30': 0,
                'scripts': 0,
                'unclaimed_rewards': 0,
            })
            self.block_sanity_check(res5['block_info'], initial_tx_height)

        # Generate and send a normal tx with two outputs
        tx1_amount = Decimal('0.21')
        tx1_txid, tx1_vout = self.wallet.send_to(
            from_node=node,
            scriptPubKey=self.wallet.get_scriptPubKey(),
            amount=int(tx1_amount * COIN),
        )

        # Find the right position of the 0.21 LIT output
        ...
        op_return_height = initial_tx_height + 6

        for hash_option in index_hash_options:
            # Check all amounts were registered correctly
            script_unspendable = Decimal(tx2_val)
            res6 = index_node.gettxoutsetinfo(hash_option, op_return_height)
            assert_equal(res6['total_unspendable_amount'], genesis_subsidy + script_unspendable)
            assert_equal(res6['block_info']['unspendable'], script_unspendable)
            assert_equal(res6['block_info']['unspendables'], {
                'genesis_block': 0,
                'bip30': 0,
                'scripts': script_unspendable,
                'unclaimed_rewards': 0,
            })
            self.block_sanity_check(res6['block_info'], op_return_height)

        # Create a coinbase that does not claim full subsidy and also
        # has two outputs
        underclaimed_height = op_return_height + 1
        underclaimed_subsidy = self.subsidy_amount(underclaimed_height)
        extra_claimed = Decimal('0.05')
        unclaimed_reward = Decimal('0.08')
        primary_claimed = underclaimed_subsidy - extra_claimed - unclaimed_reward
        cb = create_coinbase(underclaimed_height, nValue=primary_claimed)
        cb.vout.append(CTxOut(int(extra_claimed * COIN), CScript([OP_FALSE])))
        cb.rehash()
        ...
        for hash_option in index_hash_options:
            res7 = index_node.gettxoutsetinfo(hash_option, underclaimed_height)
            assert_equal(res7['total_unspendable_amount'], genesis_subsidy + script_unspendable + unclaimed_reward)
            assert_equal(res7['block_info'], {
                'unspendable': unclaimed_reward,
                'prevout_spent': 0,
                'new_outputs_ex_coinbase': 0,
                'coinbase': primary_claimed + extra_claimed,
                'unspendables': {
                    'genesis_block': 0,
                    'bip30': 0,
                    'scripts': 0,
            ...
            self.block_sanity_check(res7['block_info'], underclaimed_height)
        ...
        self.index_tip_height = underclaimed_height + 1
    ...
    def _test_use_index_option(self):
        ...
        self.nodes[0].waitforblockheight(self.index_tip_height)
    ...
    def _test_reorg_index(self):
        ...
        reorg_tip_height = base_height + 2
        res2 = index_node.gettxoutsetinfo(hash_type='muhash', hash_or_height=reorg_tip_height)
        ...
        res3 = index_node.gettxoutsetinfo(hash_type='muhash', hash_or_height=reorg_tip_height)
    ...
    def _test_index_rejects_hash_serialized(self):
        ...
        queried_height = self.nodes[1].getblockcount()
        assert_raises_rpc_error(-8, msg, self.nodes[1].gettxoutsetinfo, hash_type='hash_serialized_2', hash_or_height=queried_height)

        for use_index in {True, False, None}:
            assert_raises_rpc_error(-8, msg, self.nodes[1].gettxoutsetinfo, hash_type='hash_serialized_2', hash_or_height=queried_height, use_index=use_index)
