...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 LIT from 0 to 2 using sendtoaddress call.
        ...
        # Send 10 LIT with subtract fee from amount
        ...
        # Sendmany 10 LIT with subtract fee from amount
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Lithium address
            assert_raises_rpc_error(-5, "Invalid Lithium address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid Lithium address
            assert_raises_rpc_error(-5, "Invalid Lithium address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid Lithium address or script", self.nodes[0].importaddress, "invalid")
            ...
            assert_raises_rpc_error(
                ...
                create_deterministic_address_bcrt1_p2tr_op_true()[0],
            )
        ...
        external_address = TestNode.deterministic_address_key_pair(99).address
        address_info = self.nodes[0].getaddressinfo(external_address)
        assert_equal(address_info['address'], external_address)
        assert_equal(address_info["scriptPubKey"], address_to_scriptpubkey(external_address).hex())
        ...
        if self.options.descriptors:
            ...
            xpub_a = translate_testnet_extkey("tpubD6NzVbkrYhZ4YBNjUo96Jxd1u4XKWgnoc7LsA1jz3Yc2NiDbhtfBhaBtemB73n9V5vtJHwU6FVXwggTbeoJWQ1rzdz8ysDuQkpnaHyvnvzR")
            xpub_b = translate_testnet_extkey("tpubD6NzVbkrYhZ4YHdDGMAYGaWxMSC1B6tPRTHuU5t3BcfcS3nrF523iFm5waFd1pP3ZvJt4Jr8XmCmsTBNx5suhcSgtzpGjGMASR3tau1hJz4")
            xpub_c = translate_testnet_extkey("tpubD6NzVbkrYhZ4Y2RLiuEzNQkntjmsLpPYDm3LTRBYynUQtDtpzeUKAcb9sYthSFL3YR74cdFgF5mW8yKxv2W2CWuZDFR2dUpE5PF9kbrVXNZ")
            multi_a = descsum_create(f"wsh(multi(1,{xpub_a}/*,{xpub_b}/*))")
            multi_b = descsum_create(f"wsh(multi(1,{xpub_b}/*,{xpub_c}/*))")
