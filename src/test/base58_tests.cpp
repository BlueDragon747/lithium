...
BOOST_AUTO_TEST_CASE(base58_DecodeBase58)
{
    ...
    // Lithium uses a different Base58Check checksum than Bitcoin, so avoid
    // hard-coding Bitcoin's canonical checksum string here. Test the checksum
    // path by round-tripping a known payload and then corrupting the result.
    const std::vector<unsigned char> payload = ParseHex("971a55");
    const std::string base58check = EncodeBase58Check(payload);
    BOOST_CHECK(DecodeBase58Check(base58check, result, 100));
    BOOST_CHECK_EQUAL_COLLECTIONS(result.begin(), result.end(), payload.begin(), payload.end());

    std::string corrupted = base58check;
    corrupted.back() = corrupted.back() == '1' ? '2' : '1';
    std::string with_nul = base58check;
    with_nul.append("\0" "0IOl", 5);
    BOOST_CHECK(!DecodeBase58Check(corrupted, result, 100));
    BOOST_CHECK(!DecodeBase58Check((base58check + "0IOl"), result, 100));
    BOOST_CHECK(!DecodeBase58Check(with_nul, result, 100));
}
