...
std::shared_ptr<CBlock> MinerTestingSetup::Block(const uint256& prev_hash)
{
    ...
    // These tests care about validation ordering and reorg behavior, not PoW
    // grinding. On Lithium regtest, step synthetic blocks past the
    // min-difficulty threshold so FinalizeBlock does not spend minutes finding
    // nonces against the inherited genesis target.
    pblock->nTime = prev_block->GetBlockTime() + Params().GetConsensus().nPowTargetSpacing * 2 + 1;
    ...
    txCoinbase.vin[0].scriptSig = CScript{} << prev_block->nHeight + 1 << OP_0;
