...
static CAmount ExpectedLithiumLadderSubsidy(int nHeight)
{
    static constexpr CAmount CENT = COIN / 100;

    if (nHeight == 0)      return 5 * COIN;
...
static void TestLithiumSubsidy(const Consensus::Params& consensusParams)
{
    BOOST_CHECK_EQUAL(GetBlockSubsidy(0, consensusParams), 5 * COIN);

    const int activation_height{consensusParams.nSubsidyLadderActivationHeight};
    const std::vector<int> probe_heights{
        1, 1998, 1999, 174999, 175000, 349999, 350000, 524999,
        525000, 649999, 650000, 799999, 800000, 974999, 975000,
        1949475, 1949476, 14000000,
    };

    for (const int height : probe_heights) {
        const CAmount expected = height < activation_height ? 50 * COIN : ExpectedLithiumLadderSubsidy(height);
        BOOST_CHECK_EQUAL(GetBlockSubsidy(height, consensusParams), expected);
    }
}
...
BOOST_AUTO_TEST_CASE(block_subsidy_test)
{
    ...
    BOOST_CHECK_EQUAL(main_params->GetConsensus().nSubsidyLadderActivationHeight, 1949476);
    TestLithiumSubsidy(main_params->GetConsensus());
    TestLithiumSubsidy(testnet_params->GetConsensus());
    TestLithiumSubsidy(regtest_params->GetConsensus());
}
...
//! Test retrieval of valid assumeutxo values. Lithium regtest carries one
//! snapshot at height 110 with a Lithium-specific txoutset hash.
...
BOOST_AUTO_TEST_CASE(block_malleation)
{
...

    {
        ...
        uint256 merkle_collision_candidate;
        {
            ...
            // tree inner nodes using the same hash function. Lithium txids are
        ...
        // reproduces the original merkle root under Lithium's hash policy.
        block.vtx.clear();
