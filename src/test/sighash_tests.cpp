...
uint256 static SignatureHashOld(CScript scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    ss << txTmp << nHashType;
    // Lithium legacy pre-SegWit sighash is single SHA-256. Witness-v0 keeps
    // upstream HASH256 via the dedicated BIP143 path in interpreter.cpp.
    return ss.GetSHA256();
}
...
// Goal: exercise fixed legacy sighash edge cases from sighash.json, but compare
// against a local Lithium reference implementation rather than Bitcoin's
// serialized HASH256 fixtures.
BOOST_AUTO_TEST_CASE(sighash_from_data)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        sho = SignatureHashOld(scriptCode, *tx, nIn, nHashType);
