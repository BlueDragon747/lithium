...
// 2. Then we verify that repeating with the first set of headers in both
//    phases is successful.
// 3. Finally, we verify that the lower-work chain never leaves PRESYNC.
BOOST_AUTO_TEST_CASE(headers_sync_state)
{
    ...
    // Lithium regtest reuses the historical Lithium genesis nBits, which
    // makes brute-force nonce finding much slower than upstream's SHA256d
    // regtest. Keep the synthetic chain just long enough to exercise the
    // state transitions without spending minutes mining test headers.
    const int target_blocks = 4;
    ...
    CBlockIndex work_index;
    work_index.nBits = Params().GenesisBlock().nBits;
    const arith_uint256 per_header_work = GetBlockProof(work_index);
    const arith_uint256 chain_work = chain_start->nChainWork + per_header_work * (target_blocks - 1);
