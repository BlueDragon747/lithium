...
constexpr static unsigned char BLOCK_EXTRANONCES[]{
    8, 0, 2, 6, 7, 8, 8, 2, 4, 1, 8, 4, 3, 8, 6, 5, 5, 4, 0, 5, 3, 2, 2, 7, 2, 0, 1, 6,
    7, 4, 7, 6, 3, 2, 3, 8, 5, 3, 0, 3, 0, 2, 3, 2, 8, 2, 4, 8, 7, 3, 8, 4, 1, 6, 5, 3,
    3, 0, 8, 5, 0, 6, 6, 6, 6, 5, 0, 6, 4, 8, 6, 6, 6, 5, 8, 7, 3, 5, 2, 2, 6, 7, 4, 3,
    4, 0, 6, 3, 5, 8, 4, 8, 6, 0, 7, 7, 2, 6, 7, 7, 4, 1, 0, 6, 6, 5, 6, 7, 8, 0,
};

static void FindProofOfWork(CBlockHeader& block, const Consensus::Params& consensus)
{
    block.nNonce = 0;
...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    const CAmount LOW_PACKAGE_FEE = 1'000;
    const CAmount MEDIUM_PACKAGE_FEE = 10'000;
    const CAmount HIGH_PACKAGE_FEE = 50'000;
    const CAmount SPLIT_OUTPUT = CENT;
    ...
    tx.vout.resize(1);
    tx.vout[0].nValue = BLOCKSUBSIDY - LOW_PACKAGE_FEE;
    // This tx has a low fee.
    uint256 hashParentTx = tx.GetHash(); // save this txid for later use
    tx_mempool.addUnchecked(entry.Fee(LOW_PACKAGE_FEE).Time(Now<NodeSeconds>()).SpendsCoinbase(true).FromTx(tx));

    // This tx has a medium fee.
    tx.vin[0].prevout.hash = txFirst[1]->GetHash();
    tx.vout[0].nValue = BLOCKSUBSIDY - MEDIUM_PACKAGE_FEE;
    uint256 hashMediumFeeTx = tx.GetHash();
    tx_mempool.addUnchecked(entry.Fee(MEDIUM_PACKAGE_FEE).Time(Now<NodeSeconds>()).SpendsCoinbase(true).FromTx(tx));

    // This tx has a high fee, but depends on the first transaction
    tx.vin[0].prevout.hash = hashParentTx;
    tx.vout[0].nValue = BLOCKSUBSIDY - LOW_PACKAGE_FEE - HIGH_PACKAGE_FEE;
    uint256 hashHighFeeTx = tx.GetHash();
    tx_mempool.addUnchecked(entry.Fee(HIGH_PACKAGE_FEE).Time(Now<NodeSeconds>()).SpendsCoinbase(false).FromTx(tx));
    ...
    tx.vout[0].nValue = BLOCKSUBSIDY - LOW_PACKAGE_FEE - HIGH_PACKAGE_FEE; // 0 fee
    ...
    tx.vout[0].nValue = BLOCKSUBSIDY - LOW_PACKAGE_FEE - HIGH_PACKAGE_FEE - feeToUse;
    ...
    tx.vout.resize(2);
    tx.vout[0].nValue = BLOCKSUBSIDY - SPLIT_OUTPUT;
    tx.vout[1].nValue = SPLIT_OUTPUT;
    ...
    tx.vout[0].nValue = BLOCKSUBSIDY - SPLIT_OUTPUT - feeToUse;
    ...
    tx.vout[0].nValue = SPLIT_OUTPUT - MEDIUM_PACKAGE_FEE;
    tx_mempool.addUnchecked(entry.Fee(MEDIUM_PACKAGE_FEE).FromTx(tx));
    pblocktemplate = AssemblerForTest(tx_mempool).CreateNewBlock(scriptPubKey);
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    const CAmount BLOCKSUBSIDY = txFirst[0]->vout[0].nValue;
    const CAmount LOWFEE = 10'000;
    const CAmount HIGHFEE = CENT;
    const CAmount HIGHERFEE = 4 * CENT;
...

    {
        ...
        // subsidy changing around the active chain's actual halving boundary
        int nHeight = m_node.chainman->ActiveChain().Height();
        const int halving_height = m_node.chainman->GetConsensus().nSubsidyHalvingInterval;
        assert(halving_height > 0);
        // Lithium uses a height-tier subsidy ladder, so its halving interval is
        // intentionally unreachable. Do not allocate dummy indexes up to INT_MAX.
        if (halving_height - nHeight <= 10'000) {
    ...
    tx.vin[0].scriptSig = CScript() << OP_1;
    tx.vin[0].nSequence = m_node.chainman->ActiveChain().Tip()->nHeight - prevheights[0] + 2; // pass two blocks after the tip
    tx.vout.resize(1);
    tx.vout[0].nValue = BLOCKSUBSIDY-HIGHFEE;
    tx.vout[0].scriptPubKey = CScript() << OP_1;
    tx.nLockTime = 0;
    ...
    // Lithium 0.25.2 enforces relative locks during block assembly here, so
    // the non-final package should be rejected until we advance height/time.
    // However if we advance height by 2 and time by SEQUENCE_LOCK_TIME, all of them should be mined.
    // The extra height keeps the absolute and relative lock checks aligned under Lithium maturity.
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; ++i) {
        CBlockIndex* ancestor{Assert(m_node.chainman->ActiveChain().Tip()->GetAncestor(m_node.chainman->ActiveChain().Tip()->nHeight - i))};
        ancestor->nTime += SEQUENCE_LOCK_TIME; // Trick the MedianTimePast
    }
    std::vector<CBlockIndex*> extra_blocks;
    for (int i = 0; i < 2; ++i) {
        CBlockIndex* prev = m_node.chainman->ActiveChain().Tip();
        CBlockIndex* next = new CBlockIndex();
        next->phashBlock = new uint256(InsecureRand256());
        ...
        next->BuildSkip();
        m_node.chainman->ActiveChain().SetTip(*next);
        extra_blocks.push_back(next);
    }
    SetMockTime(m_node.chainman->ActiveChain().Tip()->GetMedianTimePast() + SEQUENCE_LOCK_TIME + 1);

    BOOST_CHECK(pblocktemplate = AssemblerForTest(tx_mempool).CreateNewBlock(scriptPubKey));
    BOOST_CHECK_EQUAL(pblocktemplate->block.vtx.size(), 5U);
    while (!extra_blocks.empty()) {
        CBlockIndex* del = extra_blocks.back();
        extra_blocks.pop_back();
        m_node.chainman->ActiveChain().SetTip(*Assert(del->pprev));
        m_node.chainman->ActiveChainstate().CoinsTip().SetBestBlock(del->pprev->GetBlockHash());
        delete del->phashBlock;
        delete del;
...
void MinerTestingSetup::TestPrioritisedMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vin[0].prevout.hash = hashParentTx;
    tx.vout[0].nValue = BLOCKSUBSIDY - LOW_PACKAGE_FEE - LOW_PACKAGE_FEE;
    uint256 hashPrioritsedChild = tx.GetHash();
    tx_mempool.addUnchecked(entry.Fee(LOW_PACKAGE_FEE).Time(Now<NodeSeconds>()).SpendsCoinbase(false).FromTx(tx));
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    // We can't make transactions until we have mature coinbase inputs.
    const int blocks_to_import = COINBASE_MATURITY + 10;
    int baseheight = 0;
    std::vector<CTransactionRef> txFirst;
    for (int block_num = 0; block_num < blocks_to_import; ++block_num) {
        const uint32_t extranonce = block_num < static_cast<int>(std::size(BLOCK_EXTRANONCES)) ? BLOCK_EXTRANONCES[block_num] : block_num + 1;
        pblocktemplate = AssemblerForTest(tx_mempool).CreateNewBlock(scriptPubKey);
        BOOST_REQUIRE(pblocktemplate);
        CBlock *pblock = &pblocktemplate->block; // pointer for convenience
        {
            LOCK(cs_main);
            const CBlockIndex* prev_tip = Assert(m_node.chainman->ActiveChain().Tip());
            // Lithium regtest starts from the historical genesis nBits. Push
            // only the first mined block past the min-difficulty threshold to
            // drop onto powLimit, then return to MTP+1 so the later sequence-
            // lock checks keep the timing assumptions used by this test.
            if (m_node.chainman->ActiveChain().Height() == 0) {
                pblock->nTime = prev_tip->GetBlockTime() + m_node.chainman->GetConsensus().nPowTargetSpacing * 2 + 1;
            } else {
                pblock->nTime = prev_tip->GetMedianTimePast() + 1;
            }
            pblock->nBits = GetNextWorkRequired(prev_tip, pblock, m_node.chainman->GetConsensus());
            CMutableTransaction txCoinbase(*pblock->vtx[0]);
            txCoinbase.nVersion = 1;
            txCoinbase.vin[0].scriptSig = CScript{} << (m_node.chainman->ActiveChain().Height() + 1) << extranonce;
            txCoinbase.vout.resize(1); // Ignore the (optional) segwit commitment for deterministic test block assembly.
            txCoinbase.vout[0].scriptPubKey = CScript();
            pblock->vtx[0] = MakeTransactionRef(std::move(txCoinbase));
            if (txFirst.empty()) baseheight = m_node.chainman->ActiveChain().Height();
            if (txFirst.size() < 4) {
                txFirst.push_back(pblock->vtx[0]);
            }
            pblock->hashMerkleRoot = BlockMerkleRoot(*pblock);
            FindProofOfWork(*pblock, m_node.chainman->GetConsensus());
        }
