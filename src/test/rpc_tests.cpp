...
static CKey MakeCompressedKey(const std::array<unsigned char, 32>& key_bytes)
{
    CKey key;
...
{
    UniValue r;
    const CKey key1 = MakeCompressedKey({
    ...
    const CKey key2 = MakeCompressedKey({
        0xb5, 0x24, 0xc2, 0x8b, 0x61, 0xc9, 0xb2, 0xc4,
    ...
    const CScript redeem_script = GetScriptForMultisig(1, {key1.GetPubKey(), key2.GetPubKey()});
    const CScript script_pub_key = GetScriptForDestination(ScriptHash(redeem_script));
    const std::string destination = EncodeDestination(PKHash(key1.GetPubKey()));
    const std::string prevout =
        "[{\"txid\":\"b4cc287e58f87cdae59417329f710f3ecd75a4ee1d2872b7248f50977c8493f3\","
        "\"vout\":1,\"scriptPubKey\":\"" + HexStr(script_pub_key) + "\","
        "\"redeemScript\":\"" + HexStr(redeem_script) + "\"}]";
...
BOOST_AUTO_TEST_CASE(json_parse_errors)
{
    ...
    // LIT addresses should fail parsing
...
BOOST_AUTO_TEST_CASE(help_example)
{
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", args), "> lithium-cli -named test foo=bar b=true n=1\n");
    ...
    // test shell escape
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b'ar"}}), "> lithium-cli -named test foo='b'''ar'\n");
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b\"ar"}}), "> lithium-cli -named test foo='b\"ar'\n");
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b ar"}}), "> lithium-cli -named test foo='b ar'\n");
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"name", obj_value}}), "> lithium-cli -named test name='{\"foo\":\"bar\",\"b\":false,\"n\":1}'\n");
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"name", arr_value}}), "> lithium-cli -named test name='[\"bar\",false,1]'\n");
