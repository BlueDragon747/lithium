    ...
    : TestingSetup{CBaseChainParams::REGTEST, extra_args, coins_db_in_memory, block_tree_db_in_memory}
{
    ...
    {
        LOCK(::cs_main);
        const auto tip_hash = m_node.chainman->ActiveChain().Tip()->GetBlockHash().ToString();
        // Lithium regtest deterministic COINBASE_MATURITY-block tip hash. The
        // donor Lithium-25.2 had its own tip hash here; switching to Lithium
        // regtest's distinct genesis and 120-block maturity produces a
        // different chain.
        if (tip_hash != "73e69cfec26c247a4672356cbab07deca1220bf524924c815d186c89e7c81451") {
            throw std::runtime_error(strprintf("unexpected TestChain100Setup tip hash: %s", tip_hash));
        }
    }
}
