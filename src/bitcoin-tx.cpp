...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " lithium-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  lithium-tx [options] <hex-tx> [commands]  Update hex-encoded Lithium transaction\n"
                "or:     lithium-tx [options] -create [commands]   Create hex-encoded Lithium transaction\n"
                "\n";
