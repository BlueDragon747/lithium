...
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
    ...
    HashWriter ss{};
    ss << txTmp << nHashType;
    // BEGIN LITHIUM: legacy pre-SegWit sighash (SIGVERSION_BASE) uses single
    // SHA-256 on Lithium, matching the single-SHA txid identity. Only this
    // path is affected; SIGVERSION_WITNESS_V0 above keeps double-SHA via
    // ss.GetHash() per BIP143 (and per user direction 2026-04-23 "switch
    // BIP143 to double sha-256 and keep single sha-256 txid"); Taproot
    // (SIGVERSION_TAPROOT / TAPSCRIPT) is routed through
    // SignatureHashSchnorr which uses BIP341 TaggedHash and is not touched
    // here.
    return ss.GetSHA256();
    // END LITHIUM
}
