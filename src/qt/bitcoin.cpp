...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load the locale matching the UI language shortcut.
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load the locale matching the full UI language tag.
...
static const char* qt_argv = "lithium-qt";
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // lithium: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    QApplication::setDesktopFileName(QAPP_DESKTOP_FILE_NAME);
    ...
    /// 6-7. Parse lithium.conf, determine network, switch to network specific
    ...
    // lithium: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
