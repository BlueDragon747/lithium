...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, BitcoinUnit unit)
{
    ...
    else
    {
        ...
        if (nNet > 0)
        {
            ...
            if (IsValidDestination(address)) {
                ...
                {
                    // Preserve the 0.15.21 transaction-details behavior:
                    // for legacy P2PKH-style incoming transactions, recover the
                    // sender address from the input scriptSig pubkey without
                    // requiring txindex or access to spent prevouts.
                    ...
                    for (const CTxIn& txin : wtx.tx->vin) {
                        ...
                        while (pc < scriptSig.end()) {
                            ...
                            if (!data.empty()) lastData = data;
                        }
                        if (lastData.size() == 33 || lastData.size() == 65) {
                            const CPubKey pubkey(lastData);
                            if (pubkey.IsValid()) {
                                fromAddresses.insert(EncodeDestination(PKHash(pubkey)));
                            }
                        }
                    }
                    ...
                    if (!fromAddresses.empty()) {
                        bool first = true;
                        for (const std::string& fromAddress : fromAddresses) {
                            if (!first) strHTML += ", ";
                            strHTML += GUIUtil::HtmlEscape(fromAddress);
                            first = false;
                        }
                        strHTML += "<br>";
                    } else {
                        strHTML += tr("unknown") + "<br>";
                    }
                    strHTML += "<b>" + tr("To") + ":</b> ";
    ...
    // Message from normal lithium:URI (lithium:123...?message=example)
