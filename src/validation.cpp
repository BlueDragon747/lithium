...
// Copyright (c) 2013-2026 The Lithium Developers
...
#if defined(NDEBUG)
# error "Lithium cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Lithium Signed Message:\n";
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header.
    // Disk rereads do not know the contextual height, so do not force the
    // genesis block back through the generic AuxPoW gate here. The only valid
    // block with a null prev-hash is genesis.
    const bool fGenesisLikeHeader = block.GetHash() == consensusParams.hashGenesisBlock || block.hashPrevBlock.IsNull();
    if (!fGenesisLikeHeader) {
...
bool ReadBlockHeaderFromDisk(CBlockHeader& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    block.SetNull();
    ...
    const bool fGenesisLikeHeader = block.GetHash() == consensusParams.hashGenesisBlock || block.hashPrevBlock.IsNull();
    if (fCheckPOW && !fGenesisLikeHeader) {
        if (!CheckAuxPowProofOfWork(block, consensusParams)) {
...
{
    if (!ReadBlockHeaderFromDisk(block, pindex->GetBlockPos(), consensusParams, fCheckPOW)) {
        return false;
    }
...
{
    // Pre-activation: preserve the 50 LIT flat payout that the 0.15.21 chain
    // mined before the ladder fork. Genesis stays at the historical 5 LIT.
    if (nHeight == 0)
        return 5 * COIN;
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    bool fUseVersionBits = false;

    for (int i = 0; i < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; i++) {
        ThresholdState state = VersionBitsState(pindexPrev, params, (Consensus::DeploymentPos)i, versionbitscache);
        if (state == THRESHOLD_LOCKED_IN || state == THRESHOLD_STARTED) {
            if (!fUseVersionBits) {
                nVersion = VERSIONBITS_TOP_BITS | CPureBlockHeader::GetBaseVersion(nVersion);
                fUseVersionBits = true;
            }
    ...
    if (params.nAuxpowChainId != 0) {
        nVersion = CPureBlockHeader::GetBaseVersion(nVersion) |
                   ((params.nAuxpowChainId & 0xFF) * CPureBlockHeader::VERSION_CHAIN_START);
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // Lithium's legacy 0.8 chain intentionally only enforced the BIP30
    ...
    // there with "ConnectBlock(): tried to overwrite transaction".
    //
    // Preserve the legacy behavior here for historical chain compatibility and
    // keep any tighter policy discussion out of consensus until after a
    // separate rollout plan exists.
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        {
            if (chainParams.GetConsensus().nAuxpowChainId != 0) {
                if ((pindex->nVersion & 0xff) > CBlockHeader::CURRENT_VERSION)
                    ++nUpgraded;
            } else {
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (block.IsAuxpow() != static_cast<bool>(block.auxpow)) {
        return state.DoS(100, false, REJECT_INVALID, "bad-auxpow", false, "auxpow flag/data mismatch");
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime, bool fCheckPOW = true)
{
    ...
    // Lithium's historical chain carries AuxPoW data before the nominal
    ...
    if (fAuxPowActive && consensusParams.fStrictChainId && block.GetChainId() != consensusParams.nAuxpowChainId)
        return state.DoS(100, false, REJECT_INVALID, "bad-chain-id", false, "block does not have our chain ID");
    ...
    const int nBaseVersion = block.GetBaseVersion();
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    if (fAccepted)
                        nLoaded++;
                    if (state.IsError()) {
                        LogPrintf("ERROR: Block import failed at height %d: %s\n", 
                                  mapBlockIndex.count(hash) ? mapBlockIndex[hash]->nHeight : -1,
