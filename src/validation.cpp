...
{
    // Lithium subsidy: forward-activated ladder. Genesis stays at the historical
    // 5 LIT. Pre-activation: 50 LIT flat (preserves the 0.15.21 chain history).
    // Post-activation: legacy 0.8 height-tier ladder. nSubsidyHalvingInterval is
    // intentionally unused (set to INT_MAX in chainparams).
    static constexpr CAmount CENT = COIN / 100;
    if (nHeight == 0)
        return 5 * COIN;
    if (nHeight < consensusParams.nSubsidyLadderActivationHeight)
        return 50 * COIN;

    // At/after activation: legacy 0.8 lithium height-tier ladder.
    if (nHeight < 1999)      return 48 * CENT;
    if (nHeight < 175000)    return 48 * COIN;
    if (nHeight < 350000)    return 24 * COIN;
    if (nHeight < 525000)    return 12 * COIN;
    if (nHeight < 650000)    return 6 * COIN;
    if (nHeight < 800000)    return 3 * COIN;
    if (nHeight < 975000)    return 150 * CENT;
    return 1 * COIN;
}
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // Lithium 0.15.21 only enforced BIP30 overwrite checks for new block/template
...
private:
    ...
    static bool IsAuxpowReservedVersionBit(const Consensus::Params& params, int bit)
    {
...
public:
    ...
    bool Condition(const CBlockIndex* pindex, const Consensus::Params& params) const override
    {
        return pindex->nHeight >= params.MinBIP9WarningHeight &&
               !IsAuxpowReservedVersionBit(params, m_bit) &&
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Preserve legacy Lithium mainnet replay before the scheduled BIP34 cleanup
    ...
    // NOTE (Lithium): the comment block below is inherited verbatim from Bitcoin
    ...
    // Bitcoin's, not Lithium's. Lithium's actual rule is set above: BIP30 is
    // disabled on mainnet below BIP34Height (1977347) via legacy_mainnet_pre_bip34
    // and enforced strictly at/after it. The IsBIP30Repeat/IsBIP30Unspendable
    // exception lists are Lithium-specific (the real legacy 0.15.21 duplicate
    // coinbases). The inherited text is kept for provenance; do not act on its
    // Bitcoin-specific heights.
    //
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    if (block.auxpow && !block.IsAuxpow()) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-auxpow", "auxpow on block with non-auxpow version");
    }

    // Context-free validation can only check native proof. AuxPoW parent proof
    // needs the block height for the activation rule and is checked below in
    // ContextualCheckBlockHeader.
    if (fCheckPOW && !block.auxpow && !block.IsAuxpow() && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams))
...
bool HasValidProofOfWork(const std::vector<CBlockHeader>& headers, const Consensus::Params& consensusParams)
{
    return std::all_of(headers.cbegin(), headers.cend(),
            [&](const auto& header) {
                return header.IsAuxpow() || CheckProofOfWork(header.GetPoWHash(), header.nBits, consensusParams);
            });
}
...
{
    // Lithium mainnet duplicate-coinbase history. These are the later blocks
    // that repeat a prior coinbase txid; ConnectBlock exempts only these exact
    // height/hash pairs from the BIP30 overwrite check.
    return (block_index.nHeight == 79927 && block_index.GetBlockHash() == uint256S("0xb61d8edfa49f0fca588ab9da6023a2a2636ffed24306402a95c17a3b11f03905")) ||
           (block_index.nHeight == 79948 && block_index.GetBlockHash() == uint256S("0x5d08876c3d9e2169643fabc22c1b9ad99e49892db3649bfe97a8fb68a7736675")) ||
           (block_index.nHeight == 80094 && block_index.GetBlockHash() == uint256S("0xbeb6bcf9e7be6ecc53951ff6a3672dc4f828581f6dd90d7c7a0a6ee013ef94f0")) ||
           (block_index.nHeight == 90728 && block_index.GetBlockHash() == uint256S("0xec985c67a6b5e1c1d39b03eb6149beb09b18f30e979582b5e703d71f661033b7"));
}

bool IsBIP30Unspendable(const CBlockIndex& block_index)
{
    // Earlier blocks whose coinbase outputs become unspendable because a later
    // block repeats the coinbase txid. DisconnectBlock and UTXO statistics use
    // this reverse side of the same historical duplicate pairs.
    return (block_index.nHeight == 79922 && block_index.GetBlockHash() == uint256S("0x4002f15216919c145ff66c5b5ec02cda98b70b8f5f675171605aceafb18f83cb")) ||
           (block_index.nHeight == 79942 && block_index.GetBlockHash() == uint256S("0xe9be258745c57973d3d896e2a7b3004925b4b283202543c5c768e655ce13e632")) ||
           (block_index.nHeight == 80090 && block_index.GetBlockHash() == uint256S("0x351536da5b4268f99addfdbe7f5e18233e76b9d98403155f71a09372767cd98a")) ||
           (block_index.nHeight == 90727 && block_index.GetBlockHash() == uint256S("0x297140c10b8cba0f1def5124565f218186e7b9ac5b774e68d5b6cb90600407c2"));
}
