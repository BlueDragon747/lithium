...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if ((pindexLast->nHeight+1) % params.DifficultyAdjustmentInterval() != 0)
    {
        if (params.fPowAllowMinDifficultyBlocks)
        {
            ...
            // back to the genesis block's inherited Lithium target.
    ...
    // Lithium preserves the 0.15.21/0.8 retarget lookback rule.
    // After height 150000, use the full interval lookback when calculating the
    // actual timespan, matching lithium-0.15.21 src/pow.cpp.
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    // BEGIN LITHIUM: Lithium difficulty adjustment limits (ported from
    // 0.15.21 src/pow.cpp:63-87 per user directive that Lithium consensus
    // should be found in the previous version 15.21, 2026-04-23.
    // The upstream policy uses symmetric 4x / 0.25x bounds; Lithium uses
...
bool PermittedDifficultyTransition(const Consensus::Params& params, int64_t height, uint32_t old_nbits, uint32_t new_nbits)
{
    ...
    if (height % params.DifficultyAdjustmentInterval() == 0) {
        // BEGIN LITHIUM: match the valid retarget envelope from
        // CalculateNextWorkRequired without rejecting blocks that 0.15.21
        // would accept. Pre-height-3500, Lithium caps difficulty increases
