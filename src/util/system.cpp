...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //lithium-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Lithium
    // macOS: ~/Library/Application Support/Lithium
    // Unix-like: ~/.lithium
#ifdef WIN32
    // Windows
...
{
    if (IsArgSet("-conf")) {
        return GetConfigFile(*this, GetPathArg("-conf"));
    }
    return GetConfigFile(*this, fs::path{BITCOIN_CONF_FILENAME});
}
