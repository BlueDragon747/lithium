...
int V1TransportDeserializer::readData(Span<const uint8_t> msg_bytes)
{
    ...
    // BEGIN LITHIUM: no streaming hasher update. Lithium computes the
    // P2P message checksum at GetMessageHash() time over the completed
    // vRecv payload using 8-round BLAKE-256, not streaming double SHA-256.
    // END LITHIUM
...
const uint256& V1TransportDeserializer::GetMessageHash() const
{
    assert(Complete());
    if (data_hash.IsNull()) {
        // BEGIN LITHIUM: P2P wire-message checksum is 8-round BLAKE-256
        // over the completed payload, not streaming double SHA-256. Reuse the
        // shared Hashblake helper rather than including sph_blake.h directly.
...
{
    // BEGIN LITHIUM: P2P outgoing-message checksum is 8-round BLAKE-256
    // rather than double SHA-256, symmetric with the receive path in
