...
const uint256& CNetMessage::GetMessageHash() const
{
    assert(complete());
    if (data_hash.IsNull())
        data_hash = Hashblake(vRecv.begin(), vRecv.begin() + hdr.nMessageSize);
    return data_hash;
}
...
#ifdef _WIN32
                boost::chrono::milliseconds(timeout.tv_usec/1000)
#else
...
void CConnman::WakeMessageHandler()
{
    {
#ifdef _WIN32
        boost::lock_guard<boost::mutex> lock(mutexMsgProc);
#else
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Lithium " + FormatFullVersion();
    ...
    while (!interruptNet)
    {
        ...
        if (addrConnect.IsValid()) {
            ...
            if (fFeeler) {
                ...
                if (!interruptNet.sleep_for(
#ifdef _WIN32
                    boost::chrono::milliseconds(randsleep)
#else
...
void CConnman::ThreadOpenAddedConnections()
{
    ...
    while (true)
    {
        ...
        if (!interruptNet.sleep_for(
#ifdef _WIN32
            boost::chrono::seconds(tried ? 60 : 2)
#else
            std::chrono::seconds(tried ? 60 : 2)
#endif
            ))
            return;
    }
...
void CConnman::ThreadMessageHandler()
{
    while (!flagInterruptMsgProc)
    {
        ...
        {
#ifdef _WIN32
        boost::unique_lock<boost::mutex> lock(mutexMsgProc);
        if (!fMoreWork) {
            condMsgProc.wait_until(lock, boost::chrono::steady_clock::now() + boost::chrono::milliseconds(100), [this] { return fMsgProcWake; });
        }
#else
...
#ifdef _WIN32
    threadSocketHandler = boost::thread(&TraceThread<std::function<void()> >, "net", std::function<void()>(std::bind(&CConnman::ThreadSocketHandler, this)));

    if (!gArgs.GetBoolArg("-dnsseed", true))
        LogPrintf("DNS seeding disabled\n");
    else
        threadDNSAddressSeed = boost::thread(&TraceThread<std::function<void()> >, "dnsseed", std::function<void()>(std::bind(&CConnman::ThreadDNSAddressSeed, this)));

    // Initiate outbound connections from -addnode
    threadOpenAddedConnections = boost::thread(&TraceThread<std::function<void()> >, "addcon", std::function<void()>(std::bind(&CConnman::ThreadOpenAddedConnections, this)));

    // Initiate outbound connections unless connect=0
    if (!gArgs.IsArgSet("-connect") || gArgs.GetArgs("-connect").size() != 1 || gArgs.GetArgs("-connect")[0] != "0")
        threadOpenConnections = boost::thread(&TraceThread<std::function<void()> >, "opencon", std::function<void()>(std::bind(&CConnman::ThreadOpenConnections, this)));

    // Process messages
    threadMessageHandler = boost::thread(&TraceThread<std::function<void()> >, "msghand", std::function<void()>(std::bind(&CConnman::ThreadMessageHandler, this)));
#else
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    uint256 hash = Hashblake(msg.data.begin(), msg.data.end());
