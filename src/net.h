...
private:
    ...
    // BEGIN LITHIUM: removed upstream `mutable CHash256 hasher;`. Lithium
    // uses 8-round BLAKE-256 for P2P message checksums, not double SHA-256.
    // The hash is now computed at GetMessageHash() time from the completed
    // vRecv payload rather than streamed — simpler and avoids maintaining a
    // streaming Blake-256 wrapper just for this one use.
    // END LITHIUM
    ...
    void Reset() {
        ...
        nDataPos = 0;
        data_hash.SetNull();
        // BEGIN LITHIUM: no streaming hasher to reset; Lithium computes
        // the message hash from vRecv at GetMessageHash() time.
        // END LITHIUM
    }
