...
Lithium is based on Photon. The first 1,999 blocks awarded a reduced amount to balance the mining field at launch. The reward halves approximately each year until all Lithium have been produced.
...
- Builds both the daemon (`lithiumd`) and Qt wallet (`lithium-qt`) natively on Ubuntu 18.04
...
Targets:
  --daemon          Daemon only (lithiumd)
  --qt              Qt wallet only (lithium-qt)
  --both            Both (default)
