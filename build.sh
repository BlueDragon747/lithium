#!/bin/bash
# =============================================================================
# Lithium 0.25.2 Build Script  -  All Platforms
#
# Single self-contained script to build Lithium daemon and/or Qt wallet
# for Linux, macOS, Windows, and AppImage.
#
# Based on the v25.2 Core codebase - uses autotools (./configure + make).
...
#
# Repository: https://github.com/SidGrip/Lithium.git (branch: lithium-25.2-auxpow-port)
# =============================================================================
...
QT_NAME="lithium-qt"
QT_WM_CLASS="Lithium-Qt"
QT_DESKTOP_ID="org.lithium.lithium-qt"
...
REPO_BRANCH="lithium-25.2-auxpow-port"
QT_LINUX_LAUNCHER_SOURCE="$SCRIPT_DIR/contrib/linux-release/lithium-qt-launcher.c"
APPIMAGE_PUBLIC_NAME="${COIN_NAME_UPPER}-${VERSION}-x86_64.AppImage"
WINDOWS_ICON_SOURCE_PNG="$SCRIPT_DIR/src/qt/res/icons/lithium.png"
WINDOWS_ICON_SOURCE_TESTNET_PNG="$SCRIPT_DIR/src/qt/res/icons/lithium_testnet.png"
WINDOWS_EXE_ICON_ICO="$SCRIPT_DIR/src/qt/res/icons/lithium_32.ico"
WINDOWS_EXE_ICON_TESTNET_ICO="$SCRIPT_DIR/src/qt/res/icons/lithium_32_testnet.ico"
WINDOWS_INSTALLER_ICON_ICO="$SCRIPT_DIR/share/pixmaps/lithium.ico"
...
sync_windows_icon_assets() {
    ...
    info "Regenerating Windows icon assets from repo lithium.png sources..."
    python3 - <<PY
...
cleanup_legacy_output_root() {
    ...
    local stale_ubuntu_dirs=("$OUTPUT_BASE"/lithium-v${VERSION}-ubuntu-*-x86_64 "$OUTPUT_BASE"/lithium-v${VERSION}-ubuntu-*-x86_64-daemon "$OUTPUT_BASE"/lithium-v${VERSION}-ubuntu-*-x86_64-qt)
    local stale_ubuntu_archives=("$OUTPUT_BASE"/lithium-v${VERSION}-ubuntu-*-x86_64.tar.gz)
    shopt -u nullglob
...
bundle_macos_transitive_dylibs() {
    ...
    local main_exe="$main_exe_dir/Lithium-Qt"
    local search_dirs=("$@")
    local pass=0
    local changed=1
...
compile_linux_qt_launcher() {
    ...
    local gcc_args=(
        ...
        -Wextra
        -no-pie
        "-DLITHIUM_QT_LAUNCH_TARGET=\"${target_rel}\""
        "-DLITHIUM_QT_USE_RUNTIME_ENV=${use_runtime_env}"
    )
    ...
    [[ -n "$launcher_cc" ]] || {
        if command -v docker >/dev/null 2>&1 && [[ -n "${DOCKER_NATIVE:-}" ]]; then
            ...
            docker run --rm \
                -u "$(id -u):$(id -g)" \
                -e LITHIUM_QT_LAUNCH_TARGET="$target_rel" \
                -e LITHIUM_QT_USE_RUNTIME_ENV="$use_runtime_env" \
                -e LITHIUM_QT_LAUNCH_OUTPUT="$output_name" \
...
"$compiler" -O2 -s -Wall -Wextra -no-pie \
    "-DLITHIUM_QT_LAUNCH_TARGET=\"${LITHIUM_QT_LAUNCH_TARGET}\"" \
    "-DLITHIUM_QT_USE_RUNTIME_ENV=${LITHIUM_QT_USE_RUNTIME_ENV}" \
    /repo/contrib/linux-release/lithium-qt-launcher.c \
    -o "/out/${LITHIUM_QT_LAUNCH_OUTPUT}"
'
        else
...
Type=Application
Name=Lithium
Comment=Lithium Cryptocurrency Wallet
Exec=lithium-qt
Icon=lithium-qt
Terminal=false
StartupNotify=true
Categories=Finance;Network;
StartupWMClass=Lithium-Qt
EOF
}
...
write_linux_release_readme() {
    ...
    local qt_copy_block="cp lithium-qt ~/.local/bin/"
    local qt_desktop_note=""

    if [[ "$ubuntu_ver" == 20.04* ]]; then
        qt_launcher_note=$'\nUbuntu 20.04 keeps `lithium-qt` as a tiny launcher beside `lithium-qt-bin` so GNOME treats it like an app instead of a shared library. Keep those two files together.\n'
        qt_copy_block=$'cp lithium-qt lithium-qt-bin ~/.local/bin/'
        qt_desktop_note=$'\nOn Ubuntu 20.04, copy `lithium-qt-bin` beside it too.\n'
    fi

    case "$target" in
        daemon)
            cat > "$readme_path" <<EOF
# Lithium v${VERSION} - Linux x86_64 (Ubuntu ${ubuntu_ver})
...
\`\`\`bash
cp lithiumd lithium-cli lithium-tx lithium-wallet lithium-util ~/.local/bin/
\`\`\`
...
On first run, a config file will be generated at \`~/.lithium/lithium.conf\` with random RPC credentials and peer nodes.
...
cp ${COIN_NAME}-256.png ~/.local/share/icons/hicolor/256x256/apps/lithium-qt.png
\`\`\`
...
\`\`\`bash
mkdir -p ~/.local/bin
cp lithiumd lithium-cli lithium-tx ~/.local/bin/
\`\`\`
...
This step only adds a Show Apps / application-menu launcher. Copy \`lithium-qt\` into \`~/.local/bin/\` first so the desktop entry resolves correctly.
${qt_desktop_note}
...
cleanup_linux_native_output_root() {
    ...
    shopt -s nullglob
    stale_paths=(
        "$OUTPUT_BASE"/lithium-v${VERSION}-ubuntu-*-x86_64
        "$OUTPUT_BASE"/lithium-v${VERSION}-ubuntu-*-x86_64-daemon
        "$OUTPUT_BASE"/lithium-v${VERSION}-ubuntu-*-x86_64-qt
        "$OUTPUT_BASE"/lithium-v${VERSION}-ubuntu-*-x86_64.tar.gz
    )
    shopt -u nullglob
...
install_linux_desktop_launcher() {
    ...
    local icon_source="$SCRIPT_DIR/src/qt/res/icons/lithium.png"
...
DEOF
    ...
    info "Desktop launcher installed  -  Lithium will appear in Activities search"
}
...
write_appimage_bundle_readme() {
    ...
    cat > "$readme_path" <<EOF
# Lithium v${VERSION} - Linux AppImage
...
if is_enabled "$BLAKE_ENABLE_ZMQ"; then
    if [ -f "${MXE_SYSROOT}/lib/pkgconfig/libzmq.pc" ] && ! pkg-config --atleast-version=4 libzmq 2>/dev/null; then
        echo ">>> Normalizing MXE libzmq pkg-config version for Lithium Core configure..."
...
# matching pkg-config files. Lithium Core Qt m4 probes the .pc names before
...
# Fix missing Qt translation files (Lithium fork does not include them)
...
${HOST}-strip src/lithiumd.exe 2>/dev/null || true
${HOST}-strip src/qt/lithium-qt.exe 2>/dev/null || true
${HOST}-strip src/lithium-cli.exe 2>/dev/null || true
${HOST}-strip src/lithium-tx.exe 2>/dev/null || true
${HOST}-strip src/lithium-wallet.exe 2>/dev/null || true
${HOST}-strip src/lithium-util.exe 2>/dev/null || true

echo ">>> Build complete!"
ls -lh src/lithiumd.exe src/qt/lithium-qt.exe src/lithium-cli.exe src/lithium-tx.exe src/lithium-wallet.exe src/lithium-util.exe 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/lithium/src/lithiumd.exe" "$output_dir/lithiumd-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-cli.exe" "$output_dir/lithium-cli-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-tx.exe" "$output_dir/lithium-tx-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-wallet.exe" "$output_dir/lithium-wallet-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-util.exe" "$output_dir/lithium-util-${VERSION}.exe" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/lithium/src/qt/lithium-qt.exe" "$output_dir/lithium-qt-${VERSION}.exe" 2>/dev/null || true
    fi
...
echo ">>> Stripping binaries..."
${HOST}-strip src/lithiumd 2>/dev/null || true
${HOST}-strip src/lithium-cli 2>/dev/null || true
${HOST}-strip src/lithium-tx 2>/dev/null || true
if [[ "'"$target"'" == "qt" || "'"$target"'" == "both" ]]; then
    ${HOST}-strip src/qt/lithium-qt 2>/dev/null || true
fi

APP_NAME="Lithium-Qt.app"
if [[ "'"$target"'" == "qt" || "'"$target"'" == "both" ]]; then
echo ">>> Creating macOS .app bundle..."
rm -rf "$APP_NAME"
mkdir -p "$APP_NAME/Contents/MacOS"
mkdir -p "$APP_NAME/Contents/Resources"
cp src/qt/lithium-qt "$APP_NAME/Contents/MacOS/Lithium-Qt"

# Generate .icns icon from lithium.png
ICONS_DIR="src/qt/res/icons"
if [ -f "$ICONS_DIR/lithium.png" ]; then
    echo ">>> Generating macOS icon from lithium.png..."
    apt-get update -qq >/dev/null 2>&1 || true
    apt-get install -y -qq python3-pil >/dev/null 2>&1 || true
    python3 -c "
from PIL import Image
img = Image.open('"'"'$ICONS_DIR/lithium.png'"'"')
...
<dict>
    ...
    <string>Lithium-Qt</string>
    <key>CFBundleIdentifier</key>
    <string>org.lithium.Lithium-Qt</string>
    <key>CFBundleName</key>
    <string>Lithium-Qt</string>
    <key>CFBundleDisplayName</key>
    <string>Lithium Core</string>
...
echo ">>> Build complete!"
ls -lh src/lithiumd src/qt/lithium-qt src/lithium-cli src/lithium-tx 2>/dev/null || true
if [[ "'"$target"'" == "qt" || "'"$target"'" == "both" ]]; then
...
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/lithium/src/lithiumd" "$output_dir/lithiumd-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-cli" "$output_dir/lithium-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-tx" "$output_dir/lithium-tx-${VERSION}" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet (.app bundle)..."
        local app_name="Lithium-Qt.app"
        rm -rf "$output_dir/$app_name" 2>/dev/null || true
        if docker cp "$container_name:/build/lithium/$app_name" "$output_dir/$app_name" 2>/dev/null; then
            ...
            docker exec "$container_name" find /build/lithium -name "*.app" -type d 2>/dev/null || true
        fi
        # Also copy raw binary for convenience
        docker cp "$container_name:/build/lithium/src/qt/lithium-qt" "$output_dir/lithium-qt-${VERSION}" 2>/dev/null || true
    fi
...
build_macos_cross() {
    ...
    docker create \
        ...
        -v "$tmpdir:/build/lithium:rw" \
        -v "$cache_root/macos-depends-built:/build/lithium/depends/built:rw" \
        -v "$cache_root/macos-depends-sources:/build/lithium/depends/sources:rw" \
        "$DOCKER_MACOS" \
        /bin/bash -lc '
set -euo pipefail
cd /build/lithium
...
# The v25.2 depends tree still uses the prebuilt LLVM 10 native_clang
...
if [[ "${MACOS_FORCE_SYSTEM_CLANG:-1}" == "1" ]]; then
    ...
    echo ">>> Using Core-style depends-managed clang/cctools for macOS."
fi
...
if [[ "$BLAKE_TARGET" == "qt" || "$BLAKE_TARGET" == "both" ]]; then
    ...
    make deploydir
    if [[ -d dist/Lithium-Qt.app ]]; then
        rm -rf Lithium-Qt.app
        cp -a dist/Lithium-Qt.app Lithium-Qt.app
    fi
fi

echo ">>> Build complete!"
ls -lh src/lithiumd src/lithium-cli src/lithium-tx 2>/dev/null || true
if [[ "$BLAKE_TARGET" == "qt" || "$BLAKE_TARGET" == "both" ]]; then
    ls -lh src/qt/lithium-qt 2>/dev/null || true
    ls -lh Lithium-Qt.app/Contents/MacOS/ 2>/dev/null || true
fi
' >/dev/null
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet (.app bundle)..."
        rm -rf "$output_dir/Lithium-Qt.app" 2>/dev/null || true
        if docker cp "$container_name:/build/lithium/Lithium-Qt.app" "$output_dir/Lithium-Qt.app" 2>/dev/null; then
            find "$output_dir/Lithium-Qt.app" -path "*/Contents/MacOS/*" -type f -exec chmod +x {} + 2>/dev/null || true
            success "macOS app bundle extracted to $output_dir/"
        else
            error "Could not find Lithium-Qt.app in container"
            docker exec "$container_name" find /build/lithium -maxdepth 2 -name "*.app" -type d 2>/dev/null || true
        fi
        docker cp "$container_name:/build/lithium/src/qt/lithium-qt" "$output_dir/lithium-qt-${VERSION}" 2>/dev/null || true
    fi
...
mkdir -p "$ICON_DIR"
if [ -f src/qt/res/icons/lithium.png ]; then
    cp src/qt/res/icons/lithium.png "$ICON_DIR/'"$COIN_NAME"'.png"
else
...
echo ">>> Stripping binaries..."
strip src/lithiumd 2>/dev/null || true
strip src/qt/lithium-qt 2>/dev/null || true
strip src/lithium-cli 2>/dev/null || true
strip src/lithium-tx 2>/dev/null || true
strip src/lithium-wallet 2>/dev/null || true
strip src/lithium-util 2>/dev/null || true

echo ">>> Build complete!"
ls -lh src/lithiumd src/qt/lithium-qt src/lithium-cli src/lithium-tx src/lithium-wallet src/lithium-util 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/lithium/src/lithiumd" "$daemon_stage/lithiumd-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-cli" "$daemon_stage/lithium-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-tx" "$daemon_stage/lithium-tx-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-wallet" "$daemon_stage/lithium-wallet-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/lithium/src/lithium-util" "$daemon_stage/lithium-util-${VERSION}" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/lithium/src/qt/lithium-qt" "$qt_stage/lithium-qt-${VERSION}" 2>/dev/null || true
    fi

    finalize_linux_native_output \
        "$ubuntu_ver" \
        "$target" \
        "$daemon_stage/lithiumd-${VERSION}" \
        "$daemon_stage/lithium-cli-${VERSION}" \
        "$daemon_stage/lithium-tx-${VERSION}" \
        "$qt_stage/lithium-qt-${VERSION}" \
        "$install_packages"

    final_output_dir="$(linux_output_dir "$ubuntu_ver")"
    docker cp "$container_name:/build/lithium/test/config.ini" "$final_output_dir/test-config.ini" 2>/dev/null || warn "Missing native Docker test/config.ini"
    docker cp "$container_name:/build/lithium/config.log" "$final_output_dir/config.log" 2>/dev/null || warn "Missing native Docker config.log"
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        cp "$daemon_stage/lithium-wallet-${VERSION}" "$final_output_dir/$WALLET_NAME" 2>/dev/null || warn "Missing Linux wallet tool artifact: $daemon_stage/lithium-wallet-${VERSION}"
        cp "$daemon_stage/lithium-util-${VERSION}" "$final_output_dir/$UTIL_NAME" 2>/dev/null || warn "Missing Linux util tool artifact: $daemon_stage/lithium-util-${VERSION}"
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        relink_native_linux_target "lithiumd" "$SCRIPT_DIR/src/lithiumd"
        relink_native_linux_target "lithium-cli" "$SCRIPT_DIR/src/lithium-cli"
        relink_native_linux_target "lithium-tx" "$SCRIPT_DIR/src/lithium-tx"
        relink_native_linux_target "lithium-wallet" "$SCRIPT_DIR/src/lithium-wallet"
        relink_native_linux_target "lithium-util" "$SCRIPT_DIR/src/lithium-util"
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Relinking native Linux Qt wallet against Berkeley DB 4.8..."
        relink_native_linux_target "qt/lithium-qt" "$SCRIPT_DIR/src/qt/lithium-qt"
    fi

    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        strip src/lithiumd src/lithium-cli src/lithium-tx src/lithium-wallet src/lithium-util 2>/dev/null || true
    fi
    ...
    finalize_linux_native_output \
        "$ubuntu_ver" \
        "$target" \
        "$SCRIPT_DIR/src/lithiumd" \
        "$SCRIPT_DIR/src/lithium-cli" \
        "$SCRIPT_DIR/src/lithium-tx" \
        "$SCRIPT_DIR/src/qt/lithium-qt" \
        "$install_packages"
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        cp "$SCRIPT_DIR/src/lithium-wallet" "$final_output_dir/$WALLET_NAME" 2>/dev/null || warn "Missing Linux wallet tool artifact: $SCRIPT_DIR/src/lithium-wallet"
        cp "$SCRIPT_DIR/src/lithium-util" "$final_output_dir/$UTIL_NAME" 2>/dev/null || warn "Missing Linux util tool artifact: $SCRIPT_DIR/src/lithium-util"
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        strip src/lithiumd src/lithium-cli src/lithium-tx src/lithium-wallet src/lithium-util 2>/dev/null || true
        cp src/lithiumd "$output_dir/lithiumd-${VERSION}"
        cp src/lithium-cli "$output_dir/lithium-cli-${VERSION}"
        cp src/lithium-tx "$output_dir/lithium-tx-${VERSION}"
        cp src/lithium-wallet "$output_dir/lithium-wallet-${VERSION}"
        cp src/lithium-util "$output_dir/lithium-util-${VERSION}"
        success "Daemon binaries in $output_dir/"
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        strip src/qt/lithium-qt 2>/dev/null || true
        cp src/qt/lithium-qt "$output_dir/lithium-qt-${VERSION}"

        rm -rf "$output_dir/$app_name"
        mkdir -p "$output_dir/$app_name/Contents/MacOS" "$output_dir/$app_name/Contents/Resources"
        cp src/qt/lithium-qt "$output_dir/$app_name/Contents/MacOS/Lithium-Qt"

        local icons_dir="$SCRIPT_DIR/src/qt/res/icons"
        if [[ -f "$icons_dir/lithium.png" ]] && command -v sips &>/dev/null && command -v iconutil &>/dev/null; then
            info "Generating macOS icon from lithium.png..."
            local iconset_root iconset_dir size size2
            iconset_root=$(mktemp -d)
            iconset_dir="$iconset_root/${COIN_NAME}.iconset"
            mkdir -p "$iconset_dir"
            for size in 16 32 128 256 512; do
                sips -z "$size" "$size" "$icons_dir/lithium.png" --out "$iconset_dir/icon_${size}x${size}.png" >/dev/null 2>&1 || true
                size2=$((size * 2))
                sips -z "$size2" "$size2" "$icons_dir/lithium.png" --out "$iconset_dir/icon_${size}x${size}@2x.png" >/dev/null 2>&1 || true
            done
...
build_native_windows() {
    ...
    # components that Lithium links against.
...
PY
    ...
    # Lithium 0.25.2 does not ship the upstream translation payloads.
    if [[ -f src/Makefile ]]; then
        sedi 's/^QT_QM.*=.*/QT_QM =/' src/Makefile
        sedi '/bitcoin_.*\.qm/d' src/Makefile
        sedi '/locale\/.*\.qm/d' src/Makefile
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        local daemon_bin="src/lithiumd.exe"
        local cli_bin="src/lithium-cli.exe"
        local tx_bin="src/lithium-tx.exe"
        [[ -f src/.libs/lithiumd.exe ]] && daemon_bin="src/.libs/lithiumd.exe"
        [[ -f src/.libs/lithium-cli.exe ]] && cli_bin="src/.libs/lithium-cli.exe"
        [[ -f src/.libs/lithium-tx.exe ]] && tx_bin="src/.libs/lithium-tx.exe"
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        local qt_bin="src/qt/lithium-qt.exe"
        [[ -f src/qt/.libs/lithium-qt.exe ]] && qt_bin="src/qt/.libs/lithium-qt.exe"
