#!/bin/bash
# =============================================================================
# Lithium 0.15.21 Build Script  -  All Platforms
#
...
#
# Repository: https://github.com/BlueDragon747/Lithium (branch: master)
# =============================================================================
...
DEOF
    ...
    info "Desktop launcher installed  -  Lithium Qt will appear in Activities search"
}
...
echo ">>> Build complete!"
ls -lh src/lithiumd.exe src/qt/lithium-qt.exe src/lithium-cli.exe src/lithium-tx.exe 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/$COIN_NAME/src/lithiumd.exe" "$output_dir/lithiumd-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/lithium-cli.exe" "$output_dir/lithium-cli-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/lithium-tx.exe" "$output_dir/lithium-tx-${VERSION}.exe" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/$COIN_NAME/src/qt/lithium-qt.exe" "$output_dir/lithium-qt-${VERSION}.exe" 2>/dev/null || true
    fi
...
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/$COIN_NAME/src/lithiumd" "$output_dir/lithiumd-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/lithium-cli" "$output_dir/lithium-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/lithium-tx" "$output_dir/lithium-tx-${VERSION}" 2>/dev/null || true
    fi
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/$COIN_NAME/src/qt/lithium-qt" "$output_dir/lithium-qt-${VERSION}" 2>/dev/null || true
    fi
...
if [[ "$BLAKE_TARGET" == "qt" || "$BLAKE_TARGET" == "both" ]]; then
    if [[ -f share/qt/Info.plist ]]; then
        sed -i \
            -e "s/Bitcoin-Qt/Lithium-Qt/g" \
            -e "s/org.bitcoinfoundation.Bitcoin-Qt/org.lithium.Lithium-Qt/g" \
            -e "s/org.bitcoin.BitcoinPayment/org.lithium.LithiumPayment/g" \
            -e "s/org.bitcoin.paymentrequest/org.lithium.paymentrequest/g" \
            -e "s/Bitcoin payment request/Lithium payment request/g" \
            -e "s/application\/x-bitcoin-payment-request/application\/x-${COIN_NAME}-payment-request/g" \
            -e "s|<string>bitcoin</string>|<string>lithium</string>|g" \
            share/qt/Info.plist
    fi
    ...
    if [[ -d Bitcoin-Qt.app ]]; then
        ...
        mv Bitcoin-Qt.app Lithium-Qt.app
        if [[ -f Lithium-Qt.app/Contents/MacOS/Bitcoin-Qt ]]; then
            mv Lithium-Qt.app/Contents/MacOS/Bitcoin-Qt Lithium-Qt.app/Contents/MacOS/Lithium-Qt
        fi
    fi
fi
...
' >/dev/null
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        if docker cp "$container_name:/build/$COIN_NAME/Lithium-Qt.app" "$output_dir/Lithium-Qt.app" 2>/dev/null; then
...
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/$COIN_NAME/src/lithiumd" "$daemon_stage/lithiumd-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/lithium-cli" "$daemon_stage/lithium-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/lithium-tx" "$daemon_stage/lithium-tx-${VERSION}" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/$COIN_NAME/src/qt/lithium-qt" "$qt_stage/lithium-qt-${VERSION}" 2>/dev/null || true
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        strip src/lithiumd src/lithium-cli src/lithium-tx 2>/dev/null || true
    fi
...
PY
    ...
    # Lithium 0.15.21 does not ship the upstream translation payloads.
    if [[ -f src/Makefile ]]; then
        sedi 's/^QT_QM.*=.*/QT_QM =/' src/Makefile
        sedi '/bitcoin_.*\.qm/d' src/Makefile
        sedi '/locale\/.*\.qm/d' src/Makefile
